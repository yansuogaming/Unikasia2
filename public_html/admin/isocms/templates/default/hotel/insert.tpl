...
<div class="box_form_insert_tour_new">
	<div class="box_info_tour_top box_top_opt_set">
		<div class="info_tour">
			<a href="{$PCMS}/admin/?mod={$mod}" class="back_list" title="{$core->get_Lang('tour_list')}"><i class="fa fa-angle-left"></i></a>
			...
			<div class="body_tour">
				...
				<p class="p_tripcode">
					<a class="go_overview" href="{$PCMS}/admin/hotel/insert/{$pvalTable}/overview">{$core->get_Lang('Go to overview')}</a></span> 
				</p>
			</div>
		</div>
		<div class="info_button">
			...
			<div class="action_tour btn_preview">
				<a class="" {if $oneItem.is_trash eq 1}style="pointer-events: none;color: rgb(204, 204, 204);border-color: rgb(204, 204, 204);background-color: rgb(255, 255, 255);cursor: not-allowed;"{/if} href="{$clsClassTable->getLink($pvalTable)}" target="_blank" title="{$clsClassTable->getTitle($pvalTable)}">{$core->get_Lang('Preview')}</a>
			</div>
			<div class="action_tour btn_delete" id="is_delete_tour">
				<a class="btn_preview_tour delete_tour_ex" type_btn="delete" href="{$PCMS_URL}/?mod={$mod}&act=delete&hotel_id={$core->encryptID($pvalTable)}{$pUrl}&page={$currentPage}" title="{$clsClassTable->getTitle($pvalTable)}">{$core->get_Lang('Delete')}</a>
			</div>
		</div>
...
{/literal}
<script type="text/javascript" src="{$URL_THEMES}/hotel/jquery.hotel.new.js?v={$upd_version}"></script>
