...
/**
*  Default action
*  @author		: Luong Tien Dung (dunglt@vietiso.com)		
*  @date		: 2011/09/20	
*  @version		: 3.5.1
*/
...
?>','',$lang);
	$lang = str_replace('
','',$lang);
	$lang = str_replace(';$_ADMINLANG','|||',$lang);
	$lang = str_replace('$_ADMINLANG','',$lang);
...
';
			for($i=0;$i<count($key);$i++){
				if($key[$i]!=''){
					...
					$val = str_replace('"','&quot;',$value[$i]);
					$lang_content .= '
$_ADMINLANG["'.str_replace('"','&quot;',$key[$i]).'"]="'.$val.'";';
				}
			}
			$lang_content .= "
?>";
