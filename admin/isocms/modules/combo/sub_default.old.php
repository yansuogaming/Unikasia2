    ...
    if (isset($_POST['submit']) && $_POST['submit'] == 'Update') {
        ...
        foreach ($_POST as $key => $val) {
			if($key=='iso-content'){
				...
				$content__= preg_replace_callback('#(<img\s(?>(?!src=)[^>])*?src=")data:image/(gif|png|jpeg);base64,([\w=+/]++)("[^>]*>)#', "data_upload_image_word_textarea", $content);
			}else{
				$tmp = explode('-', $key);
				if ($tmp[0] == 'iso') {
					if ($firstAdd == 0) {
		...
		<div class="wrap">
			<div class="fl span100">
				<div class="row-span">
					...
					<br><br>
					<input type="text" autocomplete="off" name="title" class="text_32 full-width border_aaa bold title_capitalize required" id="NewHotelTitle" placeholder="'.$core->get_Lang('ex').': '.$clsISO->getExName('Hotel').'" />
				</div>
			</div>
		</div>
...
function default_ajDeleteHotelRoom() {
    $clsHotelRoom = new HotelRoom();
    #
    $clsHotelRoom->deleteOne($_POST['hotel_room_id']);
    echo(1);
    die();
}
