...
function default_default(){
	...
	#-------End Page Divide-----------------------------------------------------------
	$allItem = $clsClassTable->getAll($cond." order by ".$orderBy.$limit,$clsClassTable->pkey.",is_trash,is_online,order_no,cruise_cat_id");
...
function default_edit(){
	global $assign_list, $_CONFIG,  $_SITE_ROOT, $mod , $_LANG_ID, $act, $menu_current, $current_page,$oneSetting;
	global $core, $clsModule, $clsButtonNav,$dbconn, $clsISO, $clsConfiguration,$pvalTable,$clsClassTable;
	$assign_list["clsModule"] = $clsModule;
	$user_id = $core->_USER['user_id'];
	#
	...
	$clsCountryLang = new _Country(); $assign_list["clsCountryLang"] = $clsCountryLang;
	...
	$clsCruiseDataProperty = new CruiseDataProperty();$assign_list["clsCruiseDataProperty"] = $clsCruiseDataProperty; 
	
	$lstCountryLang = $clsCountryLang->getAll("is_trash=0 order by language asc",$clsCountryLang->pkey);
	$assign_list["lstCountryLang"] = $lstCountryLang; unset($lstCountryLang);
	...
	$assign_list['listCruiseItinerary'] = $lstCruiseItinerary; unset($lstCruiseItinerary);
...
function default_liststore(){
	...
	$cruise_cat_id=isset($_GET['cruise_cat_id'])?intval($_GET['cruise_cat_id']):0;$assign_list["cruise_cat_id"] = $cruise_cat_id;
...
function default_ajLoadListDestination(){
	...
	if(is_array($lstDestination) && count($lstDestination) > 0){
		...
		$html .= '
		<li style="cursor:pointer; width:90px; margin-top:10px;" class="ajRemoveAllDestinationInCruise iso-button-primary"><i class="fa fa-times-circle-o"></i> '.$core->get_Lang('deleteall').'</li>';
		$html.='
...
function default_ajCaculatorCruisePriceOld(){
	...
	echo $clsISO->formatNumberToEasyRead($price_old-(($price_old*$hot_deals)/100)); die();
}
...
function default_ajaxAddMoreCruiseDestination(){
	...
	if($clsCruiseDestination->getAll("is_trash=0 and cruise_id='$cruise_id' and cruise_itinerary_id='$cruise_itinerary_id' and country_id='$country_id' and city_id='$city_id' and chauluc_id='$chauluc_id' and area_id='$area_id' and region_id='$region_id' and placetogo_id='$placetogo_id'")!=""){
		...
		$f="cruise_id,cruise_itinerary_id,country_id,region_id,city_id,order_no,val,chauluc_id,placetogo_id";
		$v="'$cruise_id','$cruise_itinerary_id','$country_id','$region_id','$city_id','$max_order','1','$chauluc_id','$placetogo_id'";
...
function default_ajaxCreateQuickCruise(){
	...
	if($tp=='F'){
		...
		if($clsCruise->getAll("slug='$slugPost'")!='' && count($clsCruise->getAll("slug='$slugPost'"))>0){
			...
			$fx = "$clsCruise->pkey,title,slug,user_id,user_id_update,reg_date,upd_date,order_no";
			$vx = "'$max_id','".addslashes($titlePost)."','".$slugPost."','$user_id','$user_id','".time()."','".time()."','1'";
			if(isset($cruise_cat_id) && intval($cruise_cat_id) > 0){
				$list_cat_id = $clsCruiseCat->getListParent($cruise_cat_id);
...
function default_ajSiteCruiseService(){
	...
	if($tp == 'L') {
		...
		<form method="post" action="" id="form-post" class="frmform formborder" enctype="multipart/form-data">
			<div class="wrap">
				<div>
					...
					<div class="row-span">
						<div class="fieldlabel" style="text-align:right"><strong>'.$core->get_Lang('price').'</strong> <span class="color_r">*</span></div>
						<div class="fieldarea">
		...
		$price = isset($_POST['price'])?$clsISO->processSmartNumber($_POST['price']):0;
...
function default_cat(){
	...
	#
	if($clsConfiguration->getValue('SiteHasCruisesCategory') == 0) {
		header('location:'.PCMS_URL.'/index.php?admin&mod='.$mod.'&message=NotPermission');
		exit();
	}
	#
			...
			<form method="post" action="" id="form-post" class="frmform formborder" enctype="multipart/form-data">
				<div class="wrap">
					<div>
						...
						if($clsConfiguration->getValue('SiteHasCruisesMultiCategory')==1){
						...
						<div class="row-span">
							<div class="fieldlabel" style="text-align:right"><strong>'.$core->get_Lang('Banner').' (WxH:1920x480)</strong> <span class="color_r">*</span></div>
							<div class="fieldarea">
...
//			print_r($pvalTable.'xxxxx'.$set); die();	
			if($clsClassTable->updateOne($pvalTable,$set)) {
				if($_POST['button']=='_EDIT'){
					...
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=updateSuccess#isotab2');
				}
			} else{
				header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=updateFailed#isotab2');
			}
		} else {
...
function default_edit_cabin(){
	...
	if(isset($_POST['submit']) && $_POST['submit'] =='Update'){
		if($pvalTable>0){
			...
			if($clsClassTable->updateOne($pvalTable,$value)){
				if($_POST['button']=='_EDIT'){
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act='.$act.'&'.$clsClassTable->pkey.'='.$_GET[$clsClassTable->pkey].$pUrl.'&message=UpdateSuccess');
				} else {
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=UpdateSuccess#isotab1');
				}
			}else{
				header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=UpdateSuccess#isotab1');
			}
			...
			if ($clsClassTable->insertOne($field, $value)) {
				if ($_POST['button'] == '_EDIT') {
					header('location:' . PCMS_URL . '/?mod=' . $mod . '&act=edit_cabin&cruise_cabin_id=' . $core->encryptID($max_id) . '&cruise_id='.$cruise_id.'&message=insertSuccess');
				} else {
					header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=UpdateSuccess#isotab1');
				}
			} else {
				header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=insertFailed#isotab1');
			}
...
function default_delete_cruise_cabin(){
	...
	if($string == '' && $pvalTable == 0)
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteFailed#isotab1');
		
	if($clsClassTable->doDelete($pvalTable)){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteSuccess#isotab1');
	}
}
function default_trash_cruise_cabin(){
	...
	if($pvalTable == "")
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=TrashFailed#isotab1');

	if($clsClassTable->updateOne($pvalTable,"is_trash='1'")){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=TrashSuccess#isotab1');
	}
}
function default_restore_cruise_cabin(){
	...
	if($pvalTable == "")
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=RestoreFailed#isotab1');

	if($clsClassTable->updateOne($pvalTable,"is_trash='0'")){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=RestoreSuccess#isotab1');
	}
}
function default_delete_cruise_itinerary(){
	...
	#
	if($string == '' && $pvalTable == 0)
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteFailed#isotab2');
		
	if($clsClassTable->doDelete($pvalTable)){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteSuccess#isotab2');
	}
}
function default_trash_cruise_itinerary(){
	...
	if($pvalTable == "")
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=TrashFailed#isotab2');

	if($clsClassTable->updateOne($pvalTable,"is_trash='1'")){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=TrashSuccess#isotab2');
	}
}
function default_restore_cruise_itinerary(){
	...
	if($pvalTable == "")
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=RestoreFailed#isotab2');

	if($clsClassTable->updateOne($pvalTable,"is_trash='0'")){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=RestoreSuccess#isotab2');
	}
}
function default_delete_itinerary(){
	...
	if($pvalTable == ""){
		if($return=='edit'){
			header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=notPermission');
		}else{
			header('location:'.PCMS_URL.'/?mod=cruise&act=itinerary&cruise_id='.$cruise_id.'&message=notPermission');
		}
	}
	if($clsClassTable->deleteOne($pvalTable)){
		...
		#
		if($return=='edit'){
			header('location: '.PCMS_URL.'/?mod=cruise&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteSuccess');
		}else{
			header('location:'.PCMS_URL.'/?mod=cruise&act=itinerary&cruise_id='.$cruise_id.'&message=DeleteSuccess');
		}
	}
}
function default_trash_cruise_video(){
	...
	if($pvalTable == "")
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=TrashFailed#isotab5');

	if($clsClassTable->updateOne($pvalTable,"is_trash='1'")){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=TrashSuccess#isotab5');
	}
}
function default_restore_cruise_video(){
	...
	if($pvalTable == "")
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=RestoreFailed#isotab5');

	if($clsClassTable->updateOne($pvalTable,"is_trash='0'")){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=RestoreSuccess#isotab5');
	}
}
function default_delete_cruise_video(){
	...
	#
	if($string == '' && $pvalTable == 0)
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteFailed#isotab5');
		
	if($clsClassTable->doDelete($pvalTable)){
		header('location:' . PCMS_URL . '/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=DeleteSuccess#isotab5');
	}
}
function default_ajOpenManageCabinPriceCruise(){
	...
	if($tp=='L'){
		...
		if($lstCruiseCabin[0][$clsCruiseCabin->pkey]!=''){
			...
			for($i=0; $i<count($lstCruiseCabin); $i++){
				...
				<tr>
					...
					for($k=0; $k<count($lstGroupSize); $k++){
						...
						<td class="text-center">
							'.$clsISO->getRate().'<br />
							<input class="text full price-In cruise_season_price fontLarge" style="width:60px; text-align:right; color:red;" cruise_cabin_id="'.$lstCruiseCabin[$i]['cruise_cabin_id'].'" group_size_id="'.$lstGroupSize[$k]['cruise_property_id'].'" sesson="high" cruise_id="'.$cruise_id.'" cruise_itinerary_id="'.$cruise_itinerary_id.'" value="'.$clsCruiseSeasonPrice->getPriceDefault($cruise_itinerary_id,$lstCruiseCabin[$i]['cruise_cabin_id'],$lstGroupSize[$k]['cruise_property_id'],'high').'" type="text"  '.($clsISO->checkContainer($lstCruiseCabin[$i]['list_group_size'],$lstGroupSize[$k]['cruise_property_id'])?'':'disabled="disabled"').'/>
						</td>';	
					}
				$html .= '</tr>';
			} 
			...
			for($k=0; $k<count($lstGroupSize); $k++){
				...
				<td class="gridheader text-center">
					<input type="checkbox" '.($clsCruiseItinerary->checkRoomTypeAvailable($cruise_itinerary_id, $lstGroupSize[$k]['cruise_property_id'],'low')?'checked="checked"':'').' name="is_hide" sesson="low" class="changeToHide changeToHide_low" cruise_itinerary_id="'.$cruise_itinerary_id.'" value="'.$lstGroupSize[$k]['cruise_property_id'].'" >
				</td>';	
			}
			// High
			for($k=0; $k<count($lstGroupSize); $k++){
				$html .= '
				<td class="gridheader text-center">
					<input type="checkbox" '.($clsCruiseItinerary->checkRoomTypeAvailable($cruise_itinerary_id, $lstGroupSize[$k]['cruise_property_id'],'high')?'checked="checked"':'').' name="is_hide" sesson="high" class="changeToHide changeToHide_high" cruise_itinerary_id="'.$cruise_itinerary_id.'" value="'.$lstGroupSize[$k]['cruise_property_id'].'" >
				</td>';	
			}
			$html .= '</tr>';
			}
		...
		if($res[0]['cruise_season_price_id'] != ''){
			...
			$f = "cruise_id,cruise_itinerary_id,cruise_cabin_id,season,price,user_id,user_id_update,reg_date,upd_date,group_size_id";
			$v = "'$cruise_id'
				,'$cruise_itinerary_id'
				,'$cruise_cabin_id'
				,'$season'
					...
					<tr>
						<td width="136px" rowspan="4" style="vertical-align:top;">
							 <div class="photobox_160 image">
								...
								'.($clsCruiseCabin->getImageUrl($cruise_cabin_id)==''?'':'<a pvalTable="'.$cruise_cabin_id.'" clsTable="CruiseCabin" href="javascript:void()" title="'.$core->get_Lang('delete').'" class="photobox_edit deleteItemImage"> X </a>').'
							</div>
						</td>
...
function default_edit_cruise_video(){
	...
	$clsCruise = new Cruise();$assign_list["clsCruise"] = $clsCruise;
	$return = isset($_GET['return'])?$_GET['return']:'';
	$cruise_id = $_GET['cruise_id'];$assign_list["cruise_id"] = $cruise_id;
	...
	if(isset($_POST['submit']) && $_POST['submit'] =='Update'){
		if($pvalTable>0){
			...
			if($clsClassTable->updateOne($pvalTable,$set)) {
				if($_POST['button']=='_EDIT'){
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act='.$act.'&'.$clsClassTable->pkey.'='.$core->encryptID($_GET[$clsClassTable->pkey]).$pUrl.'&message=updateSuccess');
				}
				else{
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$core->encryptID($cruise_id).'&message=updateSuccess#isotab5');
				}
			} else{
				header('location: '.PCMS_URL.'/?mod='.$mod.'&act=cruise_video'.$pUrl.'&message=updateFailed');
			}
		} else {
			$value = ""; $firstAdd = 0; $field = "";
			foreach($_POST as $key=>$val){
			...
			if($clsClassTable->insertOne($field,$value)){
				if ($_POST['button'] == '_EDIT') {
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act='.$act.'&'.$clsClassTable->pkey.'='.$core->encryptID($max_id).$pUrl.'&message=insertSuccess');
				}else {
					header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&cruise_id='.$cruise_id.'&message=insertSuccess#isotab5');
				}
			} else{
				header('location: '.PCMS_URL.'/?mod='.$mod.'&act=edit&'.$clsClassTable->pkey.'='.$max_id.$pUrl.'&message=insertFailed');
			}
		}
	}
}
	...
	// Edit Upload Form
		...
		<table cellpadding="2" cellspacing="2" width="100%" class="form">
			...
			<tr>
				<td class="fieldarea">
					<div class="photobox image">
						'.($clsCruiseMapImage->getOneField('image',$cruise_map_image_id)==''?'':'<a pvalTable="'.$cruise_map_image_id.'" clsTable="CruiseMapImage" href="javascript:void()" title="'.$core->get_Lang('delete').'" class="photobox_edit deleteItemImage"> X </a>').'
...
function default_property(){
	...
	if($type==''){
		header('location: '.PCMS_URL.'/index.php?mod='.$mod.'&act=property&type='.$listType['CabinFacilities'].'');
		exit();
	}
	...
	$html .= '
			<form method="post" id="frmForm" class="frmform formborder" enctype="multipart/form-data">
				<div class="wrap">
					...
					if($type=='GroupSize'){
						$html .='<div class="row-span">
						<div class="fieldlabel text-right bold"><strong>'.$core->get_Lang('Number adults').'</strong> <font class="color_r">*</font></div>
						<div class="fieldarea">
							<input class="text_32 border_aaa bold required" name="number_adult" value="'.$clsCruiseProperty->getNumberAdult($cruise_property_id).'" type="number" style="width:120px"/>
						</div>
					</div>';
...
//                $clsISO->print_pre($clsCruiseProperty->countItem("slug='$slugPost' and type='$type'"),true);die();
				...
				$fx = "$clsCruiseProperty->pkey,title,slug,intro,image,number_adult,order_no,type_group,`type`";
				$vx = "'".$clsCruiseProperty->getMaxID()."','".$titlePost."','".$slugPost."','".$introPost."','".$imagePost."','".$number_adult."','1','".$groupPost."','$type'";
//
...
//					echo('_ERROR'); die();
			...
			if($clsCruiseProperty->getAll("slug='$slugPost' and type='$type' and cruise_property_id <> '$cruise_property_id'")!=''){
				...
				$set = "title='$titlePost',slug='$slugPost',intro='$introPost',image='$imagePost',number_adult='$number_adult',class_icon='$classIcon',type='$type',type_group='$groupPost',group_id='$group_id'";
...
function default_ajCruisePriceSetup(){
	...
	if($tp=='L'){
		...
		#
		echo $Html.'$$'.$one['cruise_cabin_id'].'$$'.$one['cruise_property_id'].'$$'.$one['price'].'$$'.$cruise_price_table_id.'$$'.$one['season']; die();
	}
	else if($tp=='M'){
		$direct = isset($_POST['direct']) ? $_POST['direct']:'';
		$one = $clsCruisePriceTable->getOne($cruise_price_table_id);
...
function default_SiteCruiseItineraryDay(){
	...
	if($tp == 'L') {
		...
		$lstItem = $clsClassTable->getAll("is_trash=0 and cruise_itinerary_id='$cruise_itinerary_id' order by day asc limit 0,$number_day", $clsClassTable->pkey.',reg_date,day');
			...
			foreach($lstItem as $k => $item){
				...
				$html.='<td align="center" style="vertical-align: middle; text-align:center; width: 40px; white-space: nowrap;">
							<div class="btn-group">
								<button class="btn iso-button-standard dropdown-toggle" type="button" data-toggle="dropdown"> <i class="icon-cog"></i> <span class="caret"></span></button>
								<ul class="dropdown-menu" style="right:0px !important">
									<li><a class="clickEditItineraryDay" title="'.$core->get_Lang('edit').'" href="javascript:void();" data="'.$item[$clsClassTable->pkey].'"><i class="icon-edit"></i> <span>'.$core->get_Lang('edit').'</span></a></li>
									<li><a class="clickDeleteItinerary" title="'.$core->get_Lang('delete').'" href="javascript:void();" data="'.$item[$clsClassTable->pkey].'"><i class="icon-remove"></i> <span>'.$core->get_Lang('delete').'</span></a></li>
								</ul>
							</div>
						</td>';
				$html.='</tr>';
			...
			<div class="wrap">
				...
				<div class="fl span75 full_width_991">
					<div class="row-span">
						...
						<div class="fieldarea">
							<input class="text full required" style="width:70px" value="'.($cruise_itinerary_day_id==0?$clsClassTable->getMaxDay($cruise_itinerary_id):$clsClassTable->getOneField('day',$cruise_itinerary_day_id)).'" name="day" type="number" max="'.$clsCruiseItinerary->getOneField('number_day',$cruise_itinerary_id).'">
						</div>
					</div>
					<div class="row-span">
		...
		if($cruise_itinerary_day_id > 0) {
			...
			$v = "user_id_update='$user_id',title='$titlePost',slug='$slugPost',day='$dayPost',content='".$contentPost."',transport='".$transportPost."',transport_id='$transport_id',upd_date='".time()."',image='$imagePost',is_show_image='$is_show_image'";
			...
			$v ="'$user_id','$user_id','$titlePost','$slugPost','$dayPost','$cruise_itinerary_id','".$contentPost."','".time()."','".time()."','".$clsClassTable->getMaxOrderNo()."','".$clsClassTable->getMaxID()."','$transport_id','$transportPost','$imagePost','$is_show_image'";
