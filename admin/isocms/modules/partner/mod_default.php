<?php
function getFrame($partner_id=null){
...
//	error_reporting(E_ALL);
	...
	$pvalTable =Input::get('partner_id',0);$assign_list["pvalTable"] = $pvalTable;
	$type =Input::get('type','');$assign_list["type"] = $type;
	$cat_id =Input::get('cat_id',0);$assign_list["cat_id"] = $cat_id;
	$panel =Input::get('panel','');$assign_list["panel"] = $panel;

	$currentstep =Input::get('step','partner');
...
//error_reporting(E_ALL);
	...
	$allItem = $clsPartner->getAll($cond);
    $oneItem = $allItem[0];
//	$oneItem =$clsPartner->getOne($table_id);
	$tableName = $clsPartner->tbl;
    $pkeyTable = $clsPartner->pkey;
	$classTable                     = "Partner";
	$assign_list["classTable"] = $classTable;
	$assign_list["clsClassTable"] = $clsPartner;
    $assign_list["pkeyTable"] = $pkeyTable;
...
function default_ajActionNewPartner() {
//	ini_set('display_errors', '1');
//	ini_set('display_startup_errors', '1');
//	error_reporting(E_ALL);
    ...
    #
	$clsPartner = new Partner();
    $assign_list["clsPartner"] = $clsPartner;
    $tp = Input::post('tp');
    $type = Input::post('type','');
    $cat_id = Input::post('cat_id',0);

	$partner_id = $clsPartner->getMaxId();
	$title_voucher_new=$core->get_Lang('New Partner').' '.$partner_id;
    $results = array('result'=>'error');
    if($tp = 'S'){
		$clsISO->UpdateOrderNo('Partner');
		
		$field = $clsPartner->pkey.",user_id_update,title,slug,order_no,reg_date,upd_date,type,cat_id";
		$value = "'".$partner_id."','".$user_id."','".$title_voucher_new."','".$core->replaceSpace($title_voucher_new)."',1,'".time()."','".time()."','".$type."','".$cat_id."'";
//		$clsPartner->setDeBug(1);
        $clsPartner->insertOne($field,$value);
        
        if(!empty($type)){
            
            $link = 'partner/insert/'.$partner_id.'/BC/overview';
        }else{
            if($type != "" || $cat_id == 0){
                $link = 'partner/insert/'.$partner_id.'/overview';
            }else{
                $link = 'partner/insertcat/'.$partner_id;
                if($type != ''){
                    $link .= '/'.$type;
                }
                if($cat_id > 0){
