...
function default_default() {
    ...
    if (isset($_POST['filter']) && $_POST['filter'] == 'filter') {
        $link = '';
		if(isset($_POST['mod_page']) && !empty($_POST['mod_page']) && ($mod_page != $_POST['mod_page'])) {
			$link .= '&mod_page='.$_POST['mod_page'];
		} else {
			if(isset($mod_page) && !empty($mod_page)) {
				$link .= '&mod_page='.$mod_page;
			}
			if(isset($act_page) && !empty($act_page)) {
				$link .= '&act_page='.$act_page;
			}
		}
	...
	$lstModSlide = $clsClassTable->getListModPage();
	$assign_list["lstModSlide"] = $lstModSlide;
...
function default_ajUpdPosSortListSlide(){
	...
	foreach($order as $key=>$val){
		...
		$clsSlide->updateOne($val,"order_no='".$key."'");	
	}
}
function default_edit() {
	...
	#
	$clsForm->addInputTextArea("notool",'text', "", 'text', 255, 25, 2, 1,  "style='width:100%'");
...
require_once(DIR_MODULES . '/slide/mod_default.php');
?>