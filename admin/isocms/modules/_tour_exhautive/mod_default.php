...
function default_SiteFrmTourItinerary(){
	...
	$html.='				</div>
		...
		if($tour_itinerary_id > 0) {
			...
			$v = "user_id_update='$user_id',day='$dayPost',day2='$dayPost2',title='$titlePost',slug='$slugPost',content='".addslashes($contentPost)."',transport='$transportPost',meals='$mealsPost',upd_date='".time()."',image='$imagePost',transport_id='$transport_id',date_title='$dateTitlePost',is_show_image='$is_show_image'";
			#
			...
			if($clsTourItinerary->getAll("tour_id='$tour_id' and day='$dayPost'")>0){
				...
				$vx ="'".$max_id."','$user_id','$user_id','$dayPost','$tour_id','$titlePost','$slugPost','".addslashes($contentPost)."','$mealsPost','$transportPost','".time()."','".time()."','".addslashes($imagePost)."','$transport_id','".$clsTourItinerary->getMaxOrderNo()."','".$dateTitlePost."','".$is_show_image."'";
				#
...
function default_SiteFrmTourItinerary11(){
	...
	$html.='				</div>
		...
		#
		if($tour_itinerary_id > 0) {
			if($clsTourItinerary->getAll("tour_id='$tour_id' and day='$dayPost' day2='$dayPost2' and tour_itinerary_id<>'$tour_itinerary_id'")!=''){
				echo 'day_invalid'; die();	
			}
			...
			if($clsTourItinerary->getAll("tour_id='$tour_id' and day='$dayPost'")!=''){
				echo 'day_invalid'; die();	
			} else {
				$max_id = $clsTourItinerary->getMaxID();
...
function default_ajaxAddMoreTourDestination(){
	...
	$v="'".$clsTourDestination->getMaxID()."','$tour_id','$country_id','$region_id','$city_id','".$clsTourDestination->getMaxOrderNoByTour($tour_id)."','1','$chauluc_id','$area_id','$placetogo_id'";
...
function default_ajSiteFrmTourSizeGroup(){
	...
	if($tp == 'L') {
		...
		if($tour_option_id=='0'){
			...
			$v="'".$clsTourOption->getMaxID()."','$titlePost','$slugPost','".$clsISO->processSmartNumber($number_to)."','".$clsISO->processSmartNumber($number_from)."','".$clsTourOption->getMaxOrderNo($type)."','SIZEGROUP','$tour_property_id'";
...
function default_ajLoadTourPriceGroup(){
	...
	if($tp=='L'){
		$html= '';
		if(!empty($lstOption)){
			$html .= '<table class="table tbl-grid" style="border:1px solid #ccc; min-width:100%;">
				<thead>
					<tr>
		...
		if($res[0]['tour_price_group_id'] != ''){
			$clsTourPriceGroup->updateOne($res[0]['tour_price_group_id'],"price='".$price."'");
			if($tour_visitor_type_id==$adult_type_id){
				$SQL = "SELECT MIN(price) FROM ".DB_PREFIX."tour_price_group WHERE tour_id='$tour_id' and price > 0 and departure_date='$departure' and tour_visitor_type_id='$adult_type_id' and tour_number_group_id IN ($lstAdultSizeGroup)";
...
function default_ajAddTourOption(){
	...
	$v = "'".$title."','".$core->replaceSpace($title)."','".$target_id."','$user_id','".time()."','".time()."','$type','1'";
