<?php
function getFrame($city_id = null)
{
	...
	if ($clsISO->getCheckActiveModulePackage($package_id, 'hotel', 'default', 'default')  && $clsISO->getCheckActiveModulePackage($package_id, 'hotel', 'city_hotel', 'customize', 'default')) {
...
function default_getMainFormStep()
{
	...
	$clsCity = new City();
	$smarty->assign('clsClassTable', $clsCity);
	...
	// $list_month	=	$clsMonth->getSelectMultiMonth();
	// $clsISO->dd($list_month);

	#
	$table_id = Input::post('table_id', 0);
	$currentstep = Input::post('currentstep', '');
	$tableName = $clsCity->tbl;
	$pkeyTable = $clsCity->pkey;

	$oneItem = $clsCity->getOne($table_id);
	$smarty->assign('pvalTable', $table_id);
	$smarty->assign('oneItem', $oneItem);
	$smarty->assign('clsTable', 'City');
	$smarty->assign('clsTableGal', 'CityImage');
	#
	if ($oneItem['list_month_id'] != '||' && $oneItem['list_month_id'] != '|0|' && $oneItem['list_month_id'] != '') {
		$arr_month_id = $oneItem['list_month_id'];
		$arr_month_id = rtrim($arr_month_id, '|');
		$arr_month_id = ltrim($arr_month_id, '|0|');
		$arr_month_id = explode('|', $arr_month_id);
		$assign_list["arr_month_id"] = $arr_month_id;
	} else {
		$assign_list["arr_month_id"] = $arr_month_id = '';
	}
	...
	foreach ($frames as $okey => $frame) {
		...
		foreach ($steps as $key => $step) {
			$status = 0;
			if ($key == 'basic' && $oneItem['title'] != '' && $oneItem['continent_id'] > 0  && $oneItem['country_id'] > 0   && $oneItem['region_id'] > 0) {
				$status = 1;
			}
	...
	if ($currentstep == 'seo') {
		$clsMeta = new Meta();
		$assign_list["clsMeta"] = $clsMeta;
		$linkMeta = $clsCity->getLink($table_id);
		$allMeta = $clsMeta->getAll("config_link='$linkMeta'");
		$meta_id = $allMeta[0]['meta_id'];
...
function default_ajSaveMainStep()
{
	...
	if ($currentstep == 'basic') {
		...
		$listItem = $clsClassTable->getAll("slug='$slug' AND city_id <> '" . $table_id . "' AND continent_id='" . $continent_id . "'  AND country_id='" . $country_id . "'  AND region_id='" . $region_id . "' ", $clsClassTable->pkey);
		if (!empty($listItem)) {
			$msg = '_EXIST';
			// Output
			echo $msg;
		...
		#
		$month_ids	= 	Input::post('iso-month_id', []);
		...
		// Bỏ phần tử month_id tự fill trong mảng $arr_update
		if (array_key_exists('month_id', $arr_update)) {
			unset($arr_update['month_id']);
		}
...
function default_ajActionNewCity()
{
	//	ini_set('display_errors', '1');
	//	ini_set('display_startup_errors', '1');
	//	error_reporting(E_ALL);
	...
	if ($tp = 'S') {
		$clsISO->UpdateOrderNo('City');

		$field = $clsCity->pkey . ",user_id,user_id_update,title,slug,order_no,reg_date,upd_date";
		$value = "'" . $city_id . "','" . $user_id . "','" . $user_id . "','" . $title_voucher_new . "','" . $core->replaceSpace($title_voucher_new) . "',1,'" . time() . "','" . time() . "'";
		...
		$clsCity->insertOne($field, $value);
		$results = array('result' => 'success', 'link' => 'city/insert/' . $city_id . '/overview');
	}
	// Return
