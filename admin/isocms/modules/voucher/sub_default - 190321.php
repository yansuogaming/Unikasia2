...
function default_ajaxLoadVoucherDestination()
{
	...
	if (is_array($lstDestination) && count($lstDestination) > 0) {
		foreach ($lstDestination as $k => $v) {
			...
			$html .= '<li style="cursor:move" id="order_' . $v[$clsVoucherDestination->pkey] . '"><strong><a href="javascript:void(0);" title="' . $core->get_Lang('Drag & drop change position') . '">' . $title . '</a></strong><span class="remove removeDestination" data="' . $v[$clsVoucherDestination->pkey] . '">x</span></li>';
		}
		$html .= '
