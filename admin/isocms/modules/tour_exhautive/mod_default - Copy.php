	...
	if($tp == 'L') {
		...
		if(!empty($lstItem)){
			for($i=0, $max=count($lstItem); $i<$max; $i++){
				...
				<td class="name_service">'.$clsTourItinerary->getTitle($lstItem[$i][$clsTourItinerary->pkey]).'
...
function default_ajLoadTourPriceGroup(){
	...
	$oneItem = $clsTour->getOne($tour_id, "tour_option,adult_group_size,child_group_size,infant_group_size");
	...
	$lstChildSize = !empty($oneItem['child_group_size']) 
		? $clsISO->getArrayByTextSlash($oneItem['child_group_size']) : array();
	$lstInfantSize = !empty($oneItem['infant_group_size']) 
		? $clsISO->getArrayByTextSlash($oneItem['infant_group_size']) : array();	
	...
	if($tp=='L'){
		...
		if(!empty($lstOption)){
			foreach($lstOption as $key=>$val){
				...
				<table class="table table-iloocal table-bordered mb-0 radius-3" >
					<tr class="bg-gray">
						<td colspan="3" class="text-left bg-gray">'.$clsTourOption->getTitle($val).'</td>
					</tr>';
					foreach($lstTourVisitorType as $a=>$b){
						if($b[$clsTourProperty->pkey]==$adult_type_id && $total_adult_group_size){
							...
							foreach($lstAdultSize as $k16=>$v16){
								$html .= '<tr>
									<td width="40%" class="text-left">'.$clsTourOption->getTitle($v16).'</td>
									<td class="text-left">
		...
		if($res[0]['tour_price_group_id'] != ''){
			...
			$f = "tour_price_group_id,tour_id,tour_class_id,tour_number_group_id,tour_visitor_type_id,
			price,departure_date,user_id,user_id_update,reg_date,upd_date";
			$v = "'$max_id','$tour_id','$tour_class_id','$tour_number_group_id','$tour_visitor_type_id'
				,'".$price."','$departure','$user_id','$user_id','".time()."','".time()."'";
