	...
	if($tp == 'L') {
		...
		if(!empty($lstItem)){
			for($i=0, $max=count($lstItem); $i<$max; $i++){
				echo $lstItem[$i]['is_show_image'];die;
				...
				<td class="name_service">'.(($lstItem[$i]['is_show_image'] =='1')?'<img src="'.$clsTourItinerary->getImage($lstItem[$i][$clsTourItinerary->pkey],60,40).'" />':"").$clsTourItinerary->getTitle($lstItem[$i][$clsTourItinerary->pkey]).'
...
function default_ajLoadTourPriceGroup(){
	...
	$oneItem = $clsTour->getOne($tour_id, "tour_option,adult_group_size,child_group_size,infant_group_size,visitorage_child,visitorage_infant,visitorheight_child,visitorheight_infant");
...
//										$clsTourPriceGroup->setDeBug(1);
			...
			$html .= '<div class="price_single_supply radius-3 mb-half">
				<div class="row">
					...
					<div class="col-md-6">
						<div class="input-group">
							<input type="text" class="form-control price-In numberonly" tour_id="'.$tour_id.'" tour_class_id="'.$val.'" tour_start_date_id="'.$tour_start_date_id.'" departure="'.$departure.'" value="'.$clsTourPriceGroup->getPriceSingleSupplyAdmin($tour_id,$val,$departure,$is_agent).'" name="price_single_supply" placeholder="0.00">
							<span class="input-group-addon">'.$clsISO->getRate().'</span>
						</div>
					</div>
				</div>
		...
		if($res[0]['tour_price_group_id'] != ''){
			...
			$v = "'$max_id','$tour_id','$tour_class_id','$tour_number_group_id','$tour_visitor_type_id','$tour_visitor_age_type_id','$tour_visitor_height_type_id'
				,'".$price."','".$price_type."','$departure','$user_id','$user_id','".time()."','".time()."'";
			$clsTourPriceGroup->insertOne($f, $v);
			if($tour_visitor_type_id==$adult_type_id){
				$SQL = "SELECT MIN(price) FROM ".DB_PREFIX."tour_price_group WHERE tour_id='$tour_id' and price > 0 and departure_date='$departure' and tour_visitor_type_id='$adult_type_id' and tour_number_group_id IN ($adult_group_size)";
				$min_price = $dbconn->GetOne($SQL);
		...
		foreach($listVitorTypeChildren as $key => $value){
			...
			if($type == 'children'){
				...
				$html .= '<div class="box_select">
							<select name="child_size_group_'.$value['tour_option_id'].'[]" id="child_size_group" class="required chosen-select" multiple="multiple">';
				...
				$html .= '<div class="box_select">
							<select name="infant_size_group_'.$value['tour_option_id'].'[]" id="infant_size_group" class="required chosen-select" multiple="multiple">';
