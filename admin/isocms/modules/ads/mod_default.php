...
//	error_reporting(E_ALL);
	...
	$pvalTable =Input::get('ads_id',0);$assign_list["pvalTable"] = $pvalTable;
	$panel =Input::get('panel','');$assign_list["panel"] = $panel;

	$currentstep =Input::get('step','ads');
...
//error_reporting(E_ALL);
	...
	$oneItem =$clsAds->getOne($table_id);
	$tableName = $clsAds->tbl;
    $pkeyTable = $clsAds->pkey;
	$classTable                     = "Ads";
	$assign_list["classTable"]=$assign_list["clsTable"]= $classTable;
	$assign_list["clsClassTable"] = $clsAds;
    $assign_list["pkeyTable"] = $pkeyTable;
...
//	error_reporting(E_ALL);
	...
	if($currentstep=='ads'){
		...
		$arr_update['list_id'] = addslashes($list_id);
...
function default_ajActionNewAds() {
//	ini_set('display_errors', '1');
//	ini_set('display_startup_errors', '1');
//	error_reporting(E_ALL);
	...
	$ads_id = $clsAds->getMaxId();
	$title_voucher_new=$core->get_Lang('New Ads').' '.$ads_id;
    $results = array('result'=>'error');
    if($tp = 'S'){
		$clsISO->UpdateOrderNo('Ads');
		
		$field = $clsAds->pkey.",user_id,title,slug,order_no,reg_date,upd_date";
		$value = "'".$ads_id."','".$user_id."','".$title_voucher_new."','".$core->replaceSpace($title_voucher_new)."',1,'".time()."','".time()."'";
//		$clsAds->setDeBug(1);
        $clsAds->insertOne($field,$value);
        $results = array('result'=>'success','link'=>'ads/insert/'.$ads_id.'/overview');
    }
	// Return
