...
<div class="container-fluid">
	...
	<form method="post" id="forums" class="filterForm">
		<div class="wrap fiterbox">
			...
			<a class="btn btn-danger btn-delete-all-new" clsTable="Feedback" style="display:none;float: right;">
    	...
    	<div style="width:100%;">
            <table width="100%" cellspacing="0" class="tbl-grid table-striped table_responsive">
                <tr>
                    ...
                    <td class="gridheader text-left" style="width:300px"><strong>{$core->get_Lang('Special Requests')}</strong></td>
					...
					<td class="gridheader" style="width:6%"><strong>{$core->get_Lang('Reg date')}</strong></td>
                    <td class="gridheader" style="width:120px"><strong>{$core->get_Lang('status')}</strong></td>
                ...
                <tr class="{if $smarty.section.i.index%2 eq 0}row1{else}row2{/if}">
                    ...
                    <td style="white-space:nowrap">{$listItem[i].title}</td>
                    <td style="white-space:nowrap">{$clsClassTable->getFullName($listItem[i].feedback_id)}</td>
                    <td style="white-space:nowrap">{$listItem[i].phone}</td>
                    <td>{$FEEDBACKVALUE.Comments|strip_tags|truncate:300}</td>
                    <td style="white-space:nowrap">{$listItem[i].email}</td>
                    <td style="white-space:nowrap">{$clsCountry->getTitle($FEEDBACKVALUE.countryex_id)}</td>
					<td style="white-space:nowrap; text-align:center">{$clsISO->formatDate($listItem[i].reg_date)}</td>
