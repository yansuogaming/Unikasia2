<div class="breadcrumb">
    ...
    <a>&raquo;</a>
    <a href="{$curl}" title="{$mod}">{$core->get_Lang('Video')} {$clsVideoStore->getTitle($type)}</a>    
    <!-- // -->
    <a href="javascript:window.history.back();" class="back fr">{$core->get_Lang('back')}</a>
</div>
<div class="container-fluid">
	<div class="page-title">
		<h2>{$clsVideoStore->getTitle($type)} {$core->get_Lang('Manage')} </h2>
		<p>Chức năng quản lý danh sách các {$clsVideoStore->getTitle($type)} trong hệ thống isoCMS</p>
		<p>This function is intended to manage {$clsVideoStore->getTitle($type)} in isoCMS system</p>
	</div>
    <div class="wrap">
        <div class="hastable fl" style="width:52%">
            ...
            <table cellspacing="0" class="tbl-grid" width="100%">
                ...
                {if $listItem[0].video_id ne ''}
                {section name=i loop=$listItem}
                <tr class="{cycle values="row1,row2"}">
                	<td class="index"><input class="chkitem" name="chkid_video[]" value="{$listItem[i].video_id}" type="checkbox"></td>
                    <td class="index">{$smarty.section.i.iteration+$stt}</td>
                    <td><strong style="font-size:18px; line-height:24px">{$clsClassTable->getTitle($listItem[i].video_id)}</strong></td>
                    </td>
                    <td style="vertical-align:middle; width:65px; text-align:right; white-space:nowrap;">
                        <a class="iso-button-action" href="{$PCMS_URL}/?mod={$mod}&act={$act}&action=Add&type={$core->encryptID($type)}&video_id={$listItem[i].video_id}" title="{$core->get_Lang('Add this video')}"><i class="icon-plus-sign icon-white"></i> {$core->get_Lang('Add')}</a>
                    </td>
                </tr>
                {/section}
                {else}
            ...
            <div style="width:100%; padding:2%; background: #fff;border:1px solid #ccc">
                ...
                <a _type="{$type}" class="btn btn-success fileinput-button clickToSaveVideoStore"> 
        ...
        <div class="hastable fr" style="width:46%">
            <div class="fiterbox">
                <div class="wrap">
                    <div class="searchbox" style="float:left !important; width:100%">
                    	<h1 style="font-family:Cambria; font-weight:bold; font-style:italic;">{$clsVideoStore->getTitle($type)} {$core->get_Lang('selected')}</h1>
                    </div>
                </div>
            </div>
            <table cellspacing="0" class="tbl-grid" width="100%">
                ...
                {if $listSelected[0].video_id ne ''}
				<tbody id="SortAble">
					{section name=i loop=$listSelected}
					<tr style="cursor:move" id="order_{$listSelected[i].video_id}" class="{cycle values="row1,row2"}">
						<td class="index">{$smarty.section.i.index+1}</td>
						<td><strong style="font-size:18px; line-height:24px">{$clsClassTable->getTitle($listSelected[i].video_id)}</strong></td>
						</td>
						<td style="vertical-align: middle; width: 65px; text-align: right; white-space: nowrap;">
							<div class="btn-group">
								...
								<ul class="dropdown-menu" style="right:0px !important">
									<li><a href="{$DOMAIN_NAME}{$clsClassTable->getLink($listSelected[i].video_id)}" target="_blank" title="{$core->get_Lang('view')}"><i class="icon-eye-open"></i> <span>{$core->get_Lang('view')}</span></a></li>
									<li><a title="{$core->get_Lang('edit')}" href="{$PCMS_URL}/?mod={$mod}&act=edit&video_id={$core->encryptID($listSelected[i].video_id)}"><i class="icon-edit"></i> <span>{$core->get_Lang('edit')}</span></a></li>
									<li><a title="{$core->get_Lang('delete')}" class="clkDeleteVideoStore"  _video_store_id="{$listSelected[i].video_store_id}" href="javascript:void(0);"><i class="icon-remove"></i> <span>{$core->get_Lang('delete')}</span></a></li>
								</ul>
							</div>
						</td>
					</tr>
...
<script type="text/javascript">
$().ready(function(){
	$(document).on('click', '.clkDeleteVideoStore', function(ev){
		if(confirm(confirm_delete)){
			var $_this = $(this);
			vietiso_loading(1);
			$.ajax({
				type: "POST",
				url:path_ajax_script+'/index.php?mod='+mod+'&act=ajDeleteVideoStore',
				data: {'video_store_id': $_this.attr('_video_store_id')},
				dataType: "html",
	...
	$(document).on('click', '.clickToSaveVideoStore', function(ev){
		var _this = $(this);
		...
		var adata = {
			'list_video_id' : $('#list_selected_chkitem').val(),
			'type' : _this.attr('_type')
		};
		_this.find('span').text(loading);
		$.ajax({
			type: "POST",
			url: path_ajax_script+"/index.php?mod="+mod+"&act=ajSaveStoreForVideo",
			data: adata,
			dataType: "html",
			success: function(html){
...
<script type="text/javascript">
	$("#SortAble").sortable({
		...
		update: function(){
			...
			$.post(path_ajax_script+"/index.php?mod=video&act=ajUpdPosSortVideoStore", order, 
