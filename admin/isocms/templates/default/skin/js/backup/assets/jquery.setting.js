$().ready(function(){
	...
	$('#slb_domain').change(function(){
		var $_this = $(this);
		window.location.href = '/ucp/?mod='+mod+'&act='+act+'&domain_id='+$_this.val()+'&common_id='+$_this.find('option:selected').attr('_common_id');
	});
	$('.ajLoadCompanyInfo').live('click',function(){
		...
		$.ajax({
			...
			data: {"common_id": _this.attr('common_id')},
			dataType:'html',
			success: function(html){
				makepopup('60%','auto',html,'frmLoadCompanyInfo');
				vietiso_loading(0);
			}
		});
		return false;
	...
	$('.ajSubmitSetting').live('click',function(){
		...
		var $common_id = $_this.attr('common_id');
		if($type == 'CompanyInfo') {
			var $title = $('#title').val();
		...
		$('#frmSettingInfo').ajaxSubmit({
			...
			success:function(html){
				...
				if(html.indexOf('_SUCCESS')>=0){
					...
					alertify.success('Save setting error!');
				}
			}
		});
		return false;
	...
	$('.ajLoadSocial').live('click',function(){
		...
		$.ajax({
			...
			dataType:'html',
			success: function(html){
				makepopup('40%','auto',html,'frmLoadSocial');
				vietiso_loading(0);
			}
		});
		return false;
	...
	$('.ajLoadAddtionMeta').live('click',function(){
		...
		$.ajax({
			...
			dataType:'html',
			success: function(html){
				makepopup("40%",'auto',html,'frmLoadAddtionMeta');
				vietiso_loading(0);
			}
		});
		return false;
	...
	$('.ajLoadConfigEmail').live('click',function(){
		...
		$.ajax({
			...
			dataType:'html',
			success: function(html){
				makepopup('40%','auto',html,'frmLoadConfigEmail');
				vietiso_loading(0);
			}
		});
		return false;
	...
	$('.ajLoadConfigSmtp').live('click',function(){
		...
		$.ajax({
			...
			dataType:'html',
			success: function(html){
				makepopup('46%','auto',html,'frmLoadConfigSmtp');
				vietiso_loading(0);
			}
		});
		return false;
	...
	$('.ajaxSaveFrmConfigSmtp').live('click',function(){
		...
		var $config_smtp_host = $_form.find('input[name=config_smtp_host]');
		var $config_smtp_port = $_form.find('input[name=config_smtp_port]');
		var $config_smtp_username = $_form.find('input[name=config_smtp_username]');
		var $config_smtp_password = $_form.find('input[name=config_smtp_password]');
		var $config_smtp_from = $_form.find('input[name=config_smtp_from]');
		var $common_id = $_this.attr('common_id');
		/**/
		if($config_smtp_host.val()==''){
			alertify.error('Error !');
			$config_smtp_host.focus();
			return false;
		}
		if($config_smtp_port.val()==''){
			alertify.error('Error !');
			$config_smtp_port.focus();
			return false;
		}
		if($config_smtp_username.val()==''){
			alertify.error('Error !');
			$config_email_smtp.focus();
			return false;
		}
		if($config_smtp_password.val()==''){
			alertify.error('Error !');
		...
		/**/
		var adata = {
			'config_smtp_host' : $config_smtp_host.val(),
			'config_smtp_port' : $config_smtp_port.val(),
			'config_smtp_username' : $config_smtp_username.val(),
			'config_smtp_password' : $config_smtp_password.val(),
			'config_smtp_from' : $config_smtp_from.val(),
			'common_id' : $common_id
		};
		vietiso_loading(1);
		$.ajax({
	...
	$('.ajLoadBookingPrefix').live('click',function(){
		var $_this = $(this);
		vietiso_loading(1);
		$.ajax({
			type: 'POST',
			...
			dataType:'html',
			success: function(html){
				makepopup('36%','auto',html,'frmLoadTripValid');
				vietiso_loading(0);
			}
		});
		return false;
