...
<div class="box_form_insert_tour_new">
	<div class="box_info_tour_top box_top_opt_set">
		<div class="info_tour">
			<a href="{$PCMS}/admin/?mod=slide" class="back_list" title="{$core->get_Lang('slide_list')}"><i class="fa fa-angle-left"></i></a>
			<div class="body_tour">
				<h3 class="table-title" table_id="{$pvalTable}">{$oneItem.title}</h3>
				<p class="p_tripcode">
					<a class="go_overview" href="{$PCMS}/admin/slide/insert/{$pvalTable}/overview">{$core->get_Lang('Go to overview')}</a></span>
				</p>
			</div>
		</div>
		<div class="info_button">
			<div class="toggle_opt btn_online action_tour">
				...
				<a class="online_tour private_tour" data-clstable="Slide" data-pkey="{$clsClassTable->pkey}" data-val="0" data-sourse_id="{$pvalTable}" data-text_last="{$core->get_Lang('Public')}">{$core->get_Lang('Private')}</a>
				{else}
				<a class="online_tour" data-clstable="Slide" data-pkey="{$clsClassTable->pkey}" data-val="1" data-sourse_id="{$pvalTable}" data-text_last="{$core->get_Lang('Private')}">{$core->get_Lang('Public')}</a>
				{/if}
			</div>
