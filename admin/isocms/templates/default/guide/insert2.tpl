...
<div class="box_form_insert_tour_new">
	<div class="box_info_tour_top box_top_opt_set">
		<div class="info_tour">
			<a href="{$PCMS}/admin/?mod=guide&act=notitle" class="back_list" title="{$core->get_Lang('guide_cat_list')}"><i class="fa fa-angle-left"></i></a>
			 <img class="image_nav_tour isoman_show_image" table_id="{$pvalTable}" src="{$clsClassTable->getImage($pvalTable,80,59)}" alt="{$clsGuideCat->getTitle($oneItem.cat_id)}" width="80" height="59">
			<div class="body_tour">
				<h3 class="table-title" table_id="{$pvalTable}">{$clsGuideCat->getTitle($oneItem.cat_id)}</h3>
				<p class="p_tripcode">
					<a class="go_overview" href="{$PCMS}/admin/guide/category/insert/{$pvalTable}/overview">{$core->get_Lang('Go to overview')}</a></span> 
				</p>
			</div>
		</div>
		<div class="info_button">
			<div class="toggle_opt btn_online action_tour">
				...
				<a class="online_tour private_tour" data-clstable="Guide2" data-pkey="{$clsClassTable->pkey}" data-val="0" data-sourse_id="{$pvalTable}" data-text_last="{$core->get_Lang('Public')}">{$core->get_Lang('Private')}</a>
				{else}
				<a class="online_tour" data-clstable="Guide2" data-pkey="{$clsClassTable->pkey}" data-val="1" data-sourse_id="{$pvalTable}" data-text_last="{$core->get_Lang('Private')}">{$core->get_Lang('Public')}</a>
				{/if}
			</div>
			<div class="action_tour btn_preview">
				<a class="btn_preview_tour preview_tour_ex" {if $oneItem.is_trash eq 1}style="pointer-events: none;color: rgb(204, 204, 204);border-color: rgb(204, 204, 204);background-color: rgb(255, 255, 255);cursor: not-allowed;"{/if} href="{$clsClassTable->getLink($pvalTable)}" target="_blank" title="{$clsGuideCat->getTitle($oneItem.cat_id)}">{$core->get_Lang('Preview')}</a>
			</div>
			<div class="action_tour btn_delete" id="is_delete_tour">
				<a class="btn_preview_tour delete_tour_ex" type_btn="delete" href="{$PCMS_URL}/?mod={$mod}&act=delete2&guide2_id={$core->encryptID($pvalTable)}" title="{$clsGuideCat->getTitle($oneItem.cat_id)}">{$core->get_Lang('Delete')}</a>
			</div>
		</div>
