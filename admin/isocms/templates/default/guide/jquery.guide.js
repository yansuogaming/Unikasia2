$().ready(function () {
    $(document).on("click", ".add_new_guide", function (ev) {
...
function loadRegion($country_id, $region_id) {
    ...
    $.ajax({
        type: "POST",
        url: path_ajax_script + "/?mod=" + mod + "&act=loadRegion",
...
function loadCity($country_id, $region_id, $city_id, $cruise_id) {
    ...
    $.ajax({
        type: "POST",
        url: path_ajax_script + "/?mod=" + mod + "&act=loadCity",
        data: {
