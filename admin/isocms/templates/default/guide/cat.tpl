<div class="page_container">
	<div class="page-title d-flex">
		...
		<div class="button_right">
			<a class="btn btn-main btn-addnew add_new_guide" type="cat" title="{$core->get_Lang('Add Guide Category')}">{$core->get_Lang('Add Guide Category')}</a>
		</div>
	</div>
				...
				<div class="hastable">
					<table cellspacing="0" class="tbl-grid table-striped table_responsive" width="100%">
						...
						<tbody id="SortAble">
							...
							<tr style="cursor:move" id="order_{$guidecat_id}" class="{cycle values=" row1,row2"}">
								<td class="check_40 has-checkbox text-center">
									{if $clsISO->checkInArray('3,5,15,20',$allItem[i].guidecat_id)}
									<span style="display:inline-block; width:20px;">&nbsp;</span>
									{else}
									<input name="p_key[]" class="chkitem el-checkbox" type="checkbox" value="{$allItem[i].guidecat_id}" />{/if}
								</td>
								<td class="hiden767 text_center">{$allItem[i].guidecat_id}</td>
								...
								<td data-title="{$core->get_Lang('func')}" class="block_responsive" align="center" style="vertical-align: middle; text-align:center; width: 40px; white-space: nowrap;">
									<div class="btn-group">
										...
										<ul class="dropdown-menu" style="right:0px !important">
											...
											<li><a title="{$core->get_Lang('edit')}" href="{$PCMS_URL}/guide/category/insert/{$allItem[i].guidecat_id}/overview"><i class="icon-edit"></i> <span>{$core->get_Lang('edit')}</span></a></li>
											{if $clsISO->checkInArray('3,5,15,20',$allItem[i].guidecat_id)}
											{else}
