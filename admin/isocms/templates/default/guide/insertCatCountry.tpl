...
<div class="box_form_insert_tour_new">
	<div class="box_info_tour_top box_top_opt_set">
		<div class="info_tour">
			<div class="body_tour" style="padding-left: unset;">
				<h3 class="table-title" table_id="{$pvalTable}">{$clsGuideCat->getTitle($guidecat_id)}</h3>
				<p class="p_tripcode">
					{*<a class="go_overview" href="{$PCMS}/admin/guide/insert/{$pvalTable}/overview">{$core->get_Lang('Go to overview')}</a></span>*}
				</p>
			</div>

			<a href="{$PCMS}/admin/?mod=guide&act=category_country" class="back_list" title="{$core->get_Lang('guide_list')}" style="position: static;"><i class="fa fa-angle-left"> Back list</i></a>
		...
		{*<div class="info_button">
			<div class="toggle_opt btn_online action_tour">
				...
				<a class="online_tour private_tour" data-clstable="GuideCatStore" data-pkey="{$clsClassTable->pkey}" data-val="0" data-sourse_id="{$pvalTable}" data-text_last="{$core->get_Lang('Public')}">{$core->get_Lang('Private')}</a>
				{else}
				<a class="online_tour" data-clstable="GuideCatStore" data-pkey="{$clsClassTable->pkey}" data-val="1" data-sourse_id="{$pvalTable}" data-text_last="{$core->get_Lang('Private')}">{$core->get_Lang('Public')}</a>
				{/if}
			</div>
