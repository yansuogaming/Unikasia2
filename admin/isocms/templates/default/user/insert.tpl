...
<div class="box_form_insert_tour_new">
	<div class="box_info_tour_top box_top_opt_set">
		<div class="info_tour">
			<a href="{$PCMS}/admin/?mod=user" class="back_list" title="{$core->get_Lang('user_list')}"><i class="fa fa-angle-left"></i></a>
			<div class="body_tour">
				<h3 class="table-title" table_id="{$pvalTable}">{$oneItem.first_name} {$oneItem.last_name}</h3>
				<p class="p_tripcode">
					<a class="go_overview" href="{$PCMS}/admin/user/insert/{$pvalTable}/overview">{$core->get_Lang('Go to overview')}</a></span> 
				</p>
			</div>
		</div>
		<div class="info_button">
			<div class="toggle_opt btn_online action_tour">
				{if $oneItem.is_active ne 1}
				<a class="online_tour private_tour" data-clstable="User" data-pkey="{$clsClassTable->pkey}" data-val="0" data-sourse_id="{$pvalTable}" toField="is_active" data-text_last="{$core->get_Lang('Public')}">{$core->get_Lang('Private')}</a>
				{else}
				<a class="online_tour" data-clstable="User" data-pkey="{$clsClassTable->pkey}" data-val="1" data-sourse_id="{$pvalTable}" toField="is_active" data-text_last="{$core->get_Lang('Private')}">{$core->get_Lang('Public')}</a>
				{/if}
			</div>
			<div class="action_tour btn_delete" id="is_delete_tour">
				<a class="btn_preview_tour delete_tour_ex confirm_delete" type_btn="delete" href="{$PCMS_URL}/?mod={$mod}&act=delete&user_id={$core->encryptID($pvalTable)}{$pUrl}&page={$currentPage}" title="{$oneItem.first_name} {$oneItem.last_name}">{$core->get_Lang('Delete')}</a>
			</div>
		</div>
