...
$().ready(function() {
	...
	$('input[class=ajvClkStartDate]').live('change',function(){
		...
		$.ajax({
			type: 'POST',
			url: path_ajax_script+'/index.php?mod=tour&act=ajAddGroupStartDate',
			data:adata,
			dataType:'html',
			success: function(html){
    ...
    if ((mod == 'tour' && act == 'default') || (mod == 'tour' && act == 'liststore') || (mod == 'tour' && act == 'listtour')) {
...
function setSelectBoxDestination() {
	if(LANG_ID=='vn'){
	   if ($SiteHasGroup_Tours == 1) {
			if ($tourgroup_ID == 3) {//change by language
				if ($SiteModActive_continent == '1') {
					$('#slb_Country').hide();
					$('#slb_RegionID').hide();
					$('#slb_CityID').hide();
