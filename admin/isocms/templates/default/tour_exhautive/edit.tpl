	...
	<div class="box_info_tour_top box_top_opt_set">
		...
		<div class="info_button">
			...
			<div class="action_tour btn_preview">
				<a class="btn_preview_tour preview_tour_ex" {if $oneItem.is_trash eq 1}style="pointer-events: none;color: rgb(204, 204, 204);border-color: rgb(204, 204, 204);background-color: rgb(255, 255, 255);cursor: not-allowed;"{/if} data-href="{$clsTour->getLink($pvalTable)}" target="_blank" title="{$clsTour->getTitle($pvalTable)}">{$core->get_Lang('Preview')}</a>
			</div>
									...
									<div class="panel panel-default panel-sidebar">
										...
										<div id="{$cat_menu}_tg" class="panel-collapse collapse {if $cat_run == $cat_menu}in{/if}">
											<div class="panel-body">
												<ul class="stepbar-list">
												{section name=j loop=$child_cat_menu}
													<li {$child_cat_menu[j]}><a class="load-block" href="{$PCMS}/admin/tour/edit/{$pvalTable}/{$cat_menu}/{$child_cat_menu[j]}" id="{$child_cat_menu[j]}">{$core->get_Lang($child_cat_menu[j])}
