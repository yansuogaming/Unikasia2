	...
	<div class="row d-flex full-height">
		<div class="col-md-9">
			<div class="fill_data_box">
				<h3 class="title_box mb05">{$core->get_Lang('Cancellation Policy tour')}
				{assign var= cancellation_policy_tour value='cancellation_policy_tour'}
				{if $CHECKHELP eq 1}
				<button data-key="{$cancellation_policy_tour}" data-label="{$core->get_Lang('Cancellation Policy tour')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
				{/if}
				</h3>
				<p class="intro_box mb40">{$core->get_Lang('introcancellationpolicytour')}</p>
				<div class="form_option_tour">
					<div class="inpt_tour p-b-30">
						{if $oneItem.yield_id}
							{$oneItem.cancellation_policy|html_entity_decode}
						{else}
							<textarea style="width:100%" class="isoTextArea" id="{$clsISO->getUniqid()}" data-name="cancellation_policy" cols="255" rows="2">{$oneItem.cancellation_policy}</textarea>
						{/if}
					</div>
				</div>
		...
		<div class="col-md-3">
			<div class="instruction_fill_data_box">
				...
				<div class="content_box">
					<p class="mb0">{$clsConfiguration->getValue($cancellation_policy_tour)|html_entity_decode}</p>
				</div>
			</div>
		</div>
	</div>
