<h3 class="title_box">{$core->get_Lang('Photo Gallery')}
	{assign var= photo_gallery_cruise value='photo_gallery_cruise'}
	{assign var= help_first value=$photo_gallery_cruise}
	{if $CHECKHELP eq 1}
	<button data-key="{$photo_gallery_cruise}" data-label="{$core->get_Lang('Photo Gallery')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
	{/if}
</h3>
		...
		<div class="row">
			<div class="col-md-5 col-sm-12">
				<div class="filedrop-picker">
					...
					<input class="hidden" id="selectFile" type="file" data-options='{ldelim}"openFrom":"gallery","clsTableGal":"CruiseImage","table_id":"{$pvalTable}"{rdelim}' name="image">
					<input style="display:none" type="file" multiple name="image[]" id="ajAttachFile">
					<div class="clearfix mt-half"></div>
					<a table_id="{$pvalTable}" isoman_multiple="1" isoman_callback='isoman_gallery_callback({ldelim}"openFrom":"gallery","clsTableGal":"CruiseImage","table_id":"{$pvalTable}"{rdelim})' class="btn btn-upload-choice ajOpenDialog" isoman_for_id="image_val" isoman_name="image">{$clsISO->makeIcon('folder-o', $core->get_Lang('From library'))}</a>
				</div>
			</div>
