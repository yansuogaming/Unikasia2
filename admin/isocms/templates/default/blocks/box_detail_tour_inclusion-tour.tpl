	...
	<div class="row d-flex full-height">
		<div class="col-md-9">
			<div class="fill_data_box">
				<h3 class="title_box mb05">{$core->get_Lang('Inclusion tour')}
				{assign var= inclusion_tour value='inclusion_tour'}
				{if $CHECKHELP eq 1}
				<button data-key="{$inclusion_tour}" data-label="{$core->get_Lang('Inclusion tour')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
				{/if}
				</h3>
				<p class="intro_box mb40">{$core->get_Lang('introinclusiontour')}</p>
				<div class="form_option_tour">
					<div class="inpt_tour">
						{if $oneItem.yield_id}
							{$oneItem.inclusion|html_entity_decode}
						{else}
							<textarea style="width:100%" class="isoTextArea" id="{$clsISO->getUniqid()}" data-name="inclusion" cols="255" rows="2">{$oneItem.inclusion}</textarea>
						{/if}
					</div>
				</div>
		...
		<div class="col-md-3">
			<div class="instruction_fill_data_box">
				...
				<div class="content_box">
					<p class="mb0">{$clsConfiguration->getValue($inclusion_tour)|html_entity_decode}</p>
				</div>
			</div>
		</div>
	</div>
