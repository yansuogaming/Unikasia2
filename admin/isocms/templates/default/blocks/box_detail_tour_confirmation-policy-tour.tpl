...
{*				<p class="intro_box mb40">{$core->get_Lang('introconfirmationpolicytour')}</p>*}
				<div class="form_option_tour">
					<div class="inpt_tour p-b-30">
						...
						{else}
							<textarea style="width:100%" class="isoTextArea" id="{$clsISO->getUniqid()}" data-name="confirmation_policy" cols="255" rows="2">{$oneItem.confirmation_policy}</textarea>
						{/if}
					</div>
				</div>
		...
		<div class="col-md-3">
			<div class="instruction_fill_data_box">
				...
				<div class="content_box">
					<p class="mb0">{$clsConfiguration->getValue($confirmation_policy_tour)|html_entity_decode}</p>
				</div>
			</div>
		</div>
	</div>
