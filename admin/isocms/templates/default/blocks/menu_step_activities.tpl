			...
			<div id="{$k}" class="panel-collapse collapse {if $panel eq $k}in{else}{/if}">
				<div class="panel-body">
					<ul class="stepbar-list stepbar-list_{$pvalTable}">
						...
						<li><a href="javascript:void(0);" class="loadYieldStep{if $currentstep eq $key} active{/if} {if $step.status eq '1'}valid{/if}" data-route="{$clsISO->getLinkAdmin('activities')}/insert/{$pvalTable}/{$k}/{$key}"  data-table_id="{$pvalTable}" data-step="{$key}" panel="{$k}" title="{$step.name}">{$step.name}</a></li>
						{/foreach}
					</ul>
				</div>
			</div>
