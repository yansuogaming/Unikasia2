			...
			<div class="fill_data_box">
				...
				<div class="form_option_tour">
					<div class="form-group">
						...
						{assign var= meta_title_blog value='meta_title_blog'}
						{assign var= help_first value=$meta_title_blog}
						{if $CHECKHELP eq 1}
						<button data-key="{$meta_title_blog}" data-label="{$core->get_Lang('Meta Title')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
						{/if}
						</label>
						...
						<div class="text_help" hidden="">{$clsConfiguration->getValue($meta_title_blog)|html_entity_decode}</div>
					</div>
					<div class="form-group">
						<label class="col-form-label" for="config_value_intro">{$core->get_Lang('Meta Description')} <span class="required_red">*</span>
						{assign var= meta_description_blog value='meta_description_blog'}
						{if $CHECKHELP eq 1}
						<button data-key="{$meta_description_blog}" data-label="{$core->get_Lang('Meta Description')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
						{/if}
						</label>
						...
						<div class="text_help" hidden="">{$clsConfiguration->getValue($meta_description_blog)|html_entity_decode}</div>
					</div>
					<div class="form-group inpt_tour">
						<label class="col-form-label">{$core->get_Lang('Image Share Social')} <span class="required_red">*</span>
						{assign var= image_share_social_blog value='image_share_social_blog'}
						{if $CHECKHELP eq 1}
						<button data-key="{$image_share_social_blog}" data-label="{$core->get_Lang('Image Share Social')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
						{/if}
						</label>
						<div class="row">
							<div class="col-md-5 col-sm-12">
								<div class="filedrop-picker" onClick="loadHelp(this)">
									...
									<div class="text_help" hidden="">{$clsConfiguration->getValue($image_share_social_blog)|html_entity_decode}</div>
								</div>
							</div>
