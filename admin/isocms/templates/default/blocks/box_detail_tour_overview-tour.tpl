	...
	<div class="row d-flex full-height">
		<div class="col-md-9">
			<div class="fill_data_box">
				<h3 class="title_box mb05">{$core->get_Lang('Overview tour')}
				{assign var= overview_tour value='overview_tour'}
				{if $CHECKHELP eq 1}
				<button data-key="{$overview_tour}" data-label="{$core->get_Lang('Overview tour')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
				{/if}
				</h3>
				<p class="intro_box mb40">{$core->get_Lang('introoverviewtour')}</p>
		...
		<div class="col-md-3">
			<div class="instruction_fill_data_box">
				...
				<div class="content_box">
					<p class="mb0">{$clsConfiguration->getValue($overview_tour)|html_entity_decode}</p>
				</div>
			</div>
		</div>
	</div>
