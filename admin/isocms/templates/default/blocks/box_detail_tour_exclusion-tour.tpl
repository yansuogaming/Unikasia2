	...
	<div class="row d-flex full-height">
		<div class="col-md-9">
			<div class="fill_data_box">
				<h3 class="title_box mb05">{$core->get_Lang('Exlusion tour')}
				{assign var= exlusion_tour value='exlusion_tour'}
				{if $CHECKHELP eq 1}
				<button data-key="{$exlusion_tour}" data-label="{$core->get_Lang('Exlusion tour')}" type="button" title="Thêm mô tả" onclick="open_texthelp(this, event)" class="btn btn-xs btn-default"><i class="fa fa-plus-circle"></i></button>
				{/if}
				</h3>
				<p class="intro_box mb40">{$core->get_Lang('introexlusiontour')}</p>
				<div class="form_option_tour">
					<div class="inpt_tour p-b-30">
						{if $oneItem.yield_id}
							{$oneItem.exclusion|html_entity_decode}
						{else}
							<textarea style="width:100%" class="isoTextArea" id="{$clsISO->getUniqid()}" data-name="exclusion" cols="255" rows="2">{$oneItem.exclusion}</textarea>
						{/if}
					</div>
				</div>
		...
		<div class="col-md-3">
			<div class="instruction_fill_data_box">
				...
				<div class="content_box">
					<p class="mb0">{$clsConfiguration->getValue($exlusion_tour)|html_entity_decode}</p>
				</div>
			</div>
		</div>
	</div>
