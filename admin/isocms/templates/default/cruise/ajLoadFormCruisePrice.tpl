		...
		<tbody>
			...
			{if $priceByLow eq 0}
				{foreach from=$lstCruiseCabin item=cabin key=k}
					...
					{assign var=listGroupCabin value=$clsCruiseProperty->getAll("type='GroupSize' AND cruise_property_id  IN("|cat:$list_group_size|cat:") order by order_no ASC",'cruise_property_id,title,number_adult,is_extra_bed')}
