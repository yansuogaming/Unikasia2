<div class="breadcrumb">
    ...
    <a>&raquo;</a>
    <a href="{$curl}">{$core->get_Lang('bookingtourmanagement')}</a>
    <!-- // -->
		...
		<div class="filterbox">
			<div class="wrap">
				...
				<div class="group_buttons fr">
					<select name="status" onchange="_reload()">
					...
					<a class="btn btn-danger btn-delete-all" clsTable="Booking" type="Tour" style="display:none">
					...
					{if 1 eq 2}
					<a href="{$PCMS_URL}/?mod={$mod}&act=booking_tour" class="btn btn-warning">
						<i class="icon-folder-open icon-white"></i> <span>{$core->get_Lang('all')} ({$totalItem})</span>
					</a>
					<a href="{$PCMS_URL}/?mod={$mod}&act=booking_tour&status=1" class="btn btn-success" style="background:#06C;border-color:#06C">
						<i class="icon-folder-open icon-white"></i> <span>{$core->get_Lang('Offered')} ({$number_process})</span>
					</a>
                    <a href="{$PCMS_URL}/?mod={$mod}&act=booking_tour&status=2" class="btn btn-success" style="background:#c00000;border-color:#c00000">
						<i class="icon-warning-sign icon-white"></i> <span>{$core->get_Lang('Reviewed')} ({$number_reviewed})</span>
					</a>
					<a href="{$PCMS_URL}/?mod={$mod}&act=booking_tour&status=0" class="btn btn-success" style="background:#FC0;border-color:#FC0">
        ...
        <div class="table_list_booking">
            <table cellspacing="0" class="tbl-grid" width="100%">
                <tr>
                    ...
                    <td class="gridheader text-left" style="width:200px"><strong>{$core->get_Lang("Customer's Contact")}</strong></td>
                    <td class="gridheader text-left" style="width:100px; white-space:nowrap"><strong>{$core->get_Lang('Booking Date')}</strong></td> 
                    <td class="gridheader" style="width:100px"><strong>{$core->get_Lang('status')}</strong></td>
                    <td class="gridheader" style="width:74px"><strong>{$core->get_Lang('action')}</strong></td>
                </tr>
                <tbody>
                    ...
                    <tr {if $smarty.section.i.index%2 eq '0'}class="row1"{else}class="row2"{/if}>
						...
						<td style="white-space:nowrap" class="text-left">{$clsISO->formatDate($listItem[i].reg_date)}</td>
						{assign var=status value=$clsClassTable->getOneField('status',$listItem[i].booking_id)}
                        <td>{$clsClassTable->getBookingStatus($status)}</td>
