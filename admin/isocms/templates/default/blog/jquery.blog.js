$().ready(function(){	
    $(document).on('click', '.add_new_blog:not(".disable")', function (ev) {
        $(this).addClass('disable');
		$.ajax({
			type: "POST",
			url: path_ajax_script + '/index.php?mod=' + mod + '&act=ajActionNewBlog',
			data: {
				'tp': 'S'
			},
			dataType: "json",
	...
	/* START_Dia_Danh */
		...
		$(document).on('click', '.ajQuickAddDestination', function(ev){
			...
			if($chauluc_id > 0 || $country_id > 0 || $region_id > 0 || $city_id > 0){
				var adata = {};
