$(document).ready(function(){
	...
	moreLessClick('.clickSeemore','.More','.Less','.semoreClick','.LessClick');
	moreLessClick('.clicSeemore','.More','.Less','.semoreClick','.LessClick');
	...
	if($('.owl_slide_testimonial').length > 0){
		...
		$owl.owlCarousel({
			...
			lazyLoad:true,
			responsiveClass:true,
			navText: ["previous<span class='fa fa-angle-left'></span>","next<span class='fa fa-angle-right'></span>"],
			responsive:{
				0:{
					items:1,
					margin:0,
	...
	$(document).on('click', ".attractive_tour--content #show-more", function(ev) {
		var $_this = $(this);
	...
	$(document).on('click', ".attractive_tour--content #show-more-top-tour", function(ev) {
		var $_this = $(this);
		var _type = $_this.attr('_type');
		_Action = 'ajLoadMoreTopTourPromotion';
		if(_type=='TopTrip'){
	...
	$(document).on('click', ".attractive_tour--content #show-more-tour-professional", function(ev) {
		var $_this = $(this);
		var _type = $_this.attr('_type');
		_Action = 'ajLoadMoreTopTourPromotion';
		if(_type=='TopTrip'){
		...
		$.ajax({
			type:'POST',
			url:path_ajax_script+'/index.php?mod=homepackage&act='+_Action+'&lang='+LANG_ID,
			data:{
				"width":width,
				"page":$pageLastest,
			},
	...
	$('.addWishlist').click(function(){
		...
		$.ajax({
			...
			success: function(html){
				...
				loadTotalWishlist($_this.data('data'),$_this.data('clsTable'));
				}
			}
		});
	});
...
function setClockLastHourTour() {
	$('.clock_last_hour').each(function(){
		...
		$_this.html(event.strftime('Còn '+format))}).on('finish.countdown', function() {
		});
	});
}
function loadTotalServiceCart() {
	$.ajax({
		type: 'POST',
		url: path_ajax_script + '/index.php?mod=cart&act=ajaxGetTotalServiceCart&lang='+LANG_ID,
		dataType: 'html',
		success: function(html) {
