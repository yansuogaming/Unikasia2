jQuery(function($){
	...
	function setCheckInOut(check_in, check_out, form_container){
		//loadDepartureCruiseItinerary(cruise_cabin_id,check_in);
	...
	$(function() {
		$('.calendar-wrapper').each(function(index, el) {
			...
			$('#calendar_submit', t).click(function(event) {
				...
				vietiso_loading(1);
				 $.ajax({
					...
					data: $_adata,
					success: function(html){
						loadDepartureCruiseItinerary($('input[name=cruise_cabin_id]').val(),$('#calendar_check_in').val());
						if(html.indexOf('_invalid_date_empty') >= 0){
							alertify.error('Check in field is not empty.');
							return false;
						}
			...
			$(document).on('click','.tabchildcol a[href="#setting_availability_tab"]',function(){
				...
				loadDepartureCruiseItinerary($cruise_cabin_id,'');
			...
			$(document).on('change','select[name=cruise_cabin_id]',function(){
				var $_this = $(this);
				loadDepartureCruiseItinerary($(this).val(),$('#calendar_check_in').val());
				$_this.closest('.calendar-wrapper').data('cruise_cabin_id',$_this.val());
                hotel.calendar.fullCalendar( 'refetchEvents' );
            });
			$(document).on('change','input[name=check_in]',function(){
				loadDepartureCruiseItinerary($('input[name=cruise_cabin_id]').val(),$(this).val());
                hotel.calendar.fullCalendar( 'refetchEvents' );
            });
		...
		function loadDepartureCruiseItinerary($cruise_cabin_id,$check_in){
			$.ajax({
				'type': 'POST',
				'url' : path_ajax_script+'/index.php?mod=cruise&act=loadDepartureCruiseItinerary',
				'data' : {"cruise_cabin_id":$cruise_cabin_id,"check_in":$check_in,"target_id":$cruise_itinerary_id},
				'dataType': 'html',
				'success':function(html){
					$('#DepartureCruiseItinerary #loadTableSettingPrice').html(html);
				}
			});
		}
	});
