...
class elFinder {
	...
	protected function mkdir($args) {
		$target = $args['target'];
		$name   = $this->convert_utf8($args['name']);
	...
	protected function upload($args) {
         ...
         * kkjkjkjk
         */
        $trees = $this->tree($args);
        $parents = $this->parents($args);
        $arrHashCheck = array();
        if(!empty($parents['tree'])){
            foreach($parents['tree'] AS $item_parent){
                $arrHashCheck[$item_parent['hash']] = $item_parent;
            }
        }
        if(!empty($trees['tree'])){
            $item = 0;
            $totalTree = count($trees['tree']);
            if($totalTree > 1)
                foreach($trees['tree'] AS $item_tree){
                    ++$item;
                    if($item < $totalTree)
                        $arrHashCheck[$item_tree['hash']] = $item_tree;                    
                }
            elseif($totalTree == 1)
                foreach($trees['tree'] AS $item_tree){
                    $arrHashCheck[$item_tree['hash']] = $item_tree;                    
        ...
        $path = '';
        if(!empty($arrHashCheck))
            foreach($arrHashCheck AS $item_hash)
                $path .= $item_hash['name'].'/';
        //echo $path;
		foreach ($files['name'] as $i => $name) {
            $name = $this->clean_file_name($name);
...
//                $path_insert = $path;
//                $file_name = $file["name"];
//                $filePathCurrent = STORE_DATA.'FileData'.DS.$path_insert.$file_name;
//                
//                include_once SYSTEM_PATH.'Image_editor.php';
//                $configwater['source_image'] = $filePathCurrent;
...
//                $configwater['wm_opacity'] = 100;
//                                
//                $configwater['wm_overlay_path'] = STORE_DATA.'warter_mark.png';
//                $configwater['wm_vrt_alignment'] = 'bottom';
//                $configwater['wm_hor_alignment'] = 'right';
//                $configwater['wm_padding'] = '-10';
//                $configwater['wm_x_transp'] = 4;
//                $configwater['wm_y_transp'] = 3;
//                $imageLib = new Image_editor($configwater);
//                $imageLib->watermark();
//                
//            }
		}
	...
	protected function convert_utf8($str)
    { 
        $utf82abc=array(
            'à'=>'a',
            'á'=>'a', 
