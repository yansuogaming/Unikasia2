...
//        $arr = array_merge($this->options, $opts);
...
//        var_dump($arr);
	...
	public function init() {
        ...
        $this->options['pass'] = '1234'; 
        $this->options['db']   = 'elfinder'; 
        $this->options['files_table'] = 'browse_file';*/
        $this->options['host'] = '10.0.0.21'; 
        $this->options['user'] = 'root'; 
        $this->options['pass'] = 'abc123'; 
        $this->options['db']   = '1top'; 
        $this->options['files_table'] = 'browse_file';
        
		if (!$this->options['host'] ||  !$this->options['user'] ||  !$this->options['pass'] ||  !$this->options['db']) 
        {
			return false;
		}
...
//			 echo mysqli_error();
    ...
    /**
    * get info dir, file in database
    * 
    */
    public function totalCurrStorage(){
        $sql = 'SELECT * FROM '.$this->tbf; 
        $totalStorage = 0;
        $totalFolder = 0; 
        $totalSubFolder = 0; 
        $result = array();
        if ($res = $this->db->query($sql)) {
            while ($data = $res->fetch_assoc()) {
                ...
                if($data["mime"] == "directory")
                {
                    ...
                    if($data["level"] > $totalSubFolder)
                        $totalSubFolder = $data["level"];  
                }      
                $totalStorage = $totalStorage +  $data["size"];                    
            }
            $result["totalStorage"] = $totalStorage;
            $result["totalFolder"] = $totalFolder;
            $result["totalSubFolder"] = $totalSubFolder;
        }                                               
        return $result;      
    }
	/**
    * check file, dir exist in database
    * 
    * @param mixed $keycode string
    */
    public function getIdFileOrDir($keycode)
    {
        $sql = "SELECT id FROM `".$this->tbf."` WHERE `keycode`='".$keycode."'";
        if ($res = $this->db->query($sql)) {
    ...
    */
    ...
    {                                          
        $sql = "DELETE FROM `".$this->tbf."` WHERE id='".$id."'";
        $this->db->query($sql);
    }
    
    public function deleteMultiItem($arr)
    {
        if(count($arr)>0)
        {
            $str = implode(",", $arr);    
            $sql = "DELETE FROM `".$this->tbf."` WHERE id IN (".$str.")";
//            var_dump($sql);
            $this->db->query($sql);
        }
        else
    ...
    * @return mixed
    */
    public function getParentItem($keycode)
    {
        $sql = "SELECT id FROM `".$this->tbf."` WHERE `keycode`='".$keycode."'";
//        var_dump($sql);
        if ($res = $this->db->query($sql)) {
    ...
    public function getSubParent($parent_id=0,$str="")
    {
        $sql = "SELECT * FROM `".$this->tbf."` WHERE parent_id = ".$parent_id;
//        var_dump($sql);  
//        var_dump($result);      
//        $result = array();   
...
//                $result[$data["id"]] = $data["id"];
                $str .= $data["id"].",";    
                $str = $this->getSubParent($data["id"],$str);   
            }
        }    
//        var_dump($str);  
        return $str;
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	public function _mkdir($path, $name) {
        /*if (!$this->_isDir($path)) {
			return false;
		}*/
		$this->clearstat();
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	public function _mkfile($path, $name) {
		/*if (!$this->_isDir($path)) {
			return false;
		}*/
		$this->clearstat();
