...
error_reporting(E_ALL & ~E_STRICT);//E_ALL
	...
	$assign_list['lst_country_id']=$lstCountryEx[0][$clsCountryEx->pkey];
	$assign_list['lstCountryEx']=$lstCountryEx; unset($lstCountryEx);
	...
	if(!empty($_frontIsLoggedin_user_id)) {
		...
		$email = $clsMember->getEmail($_frontIsLoggedin_user_id); $assign_list['email']=$email;
		$phone = $clsMember->getPhone($_frontIsLoggedin_user_id); $assign_list['phone']=$phone;
		$country__id = $clsMember->getOneField('country_id',$_frontIsLoggedin_user_id); $assign_list['country__id']=$country__id;
		$title = $clsMember->getOneField('title',$_frontIsLoggedin_user_id); $assign_list['title']=$title;
	}
	#
	$errMsg='';
	if(isset($_POST['plantrip'])&&$_POST['plantrip']=='plantrip'){
		$name 				= Input::post('name',''); 				$assign_list['name'] = $name;
		$email 				= Input::post('email',''); 				$assign_list['email'] = $email;
		$phone 				= Input::post('phone',''); 				$assign_list['phone'] = $phone;
		$country__id 		= Input::post('country__id',''); 		$assign_list['country__id'] = $country__id;
		$please 			= Input::post('please',''); 			$assign_list['please'] = $please;
		$adult_simple 		= Input::post('adult_simple',''); 		$assign_list['number_adult'] = $adult_simple;
		$children_simple 	= Input::post('children_simple',''); 	$assign_list['number_child'] = $children_simple;
		$number_room 		= Input::post('number_room',''); 		$assign_list['number_room'] = $number_room;
		/*if($title==''){
