...
function default_detail(){
	...
	$oneTable = $clsCruise->getOne($cruise_id,'title,slug,about,inclusion,exclusion,cruise_policy,booking_policy,child_policy,departure_port,star_number,image,important_notes,listThingAbout,listCruiseBudget,listTravelAs,listRestFa,listCruiseFacilities,listCruiseFaActivities,listCruiseServices,cruise_cat_id,price_3day,price,build,total_cabin,material');$assign_list["oneTable"] = $oneTable;
...
function default_cat(){
	...
	if(isset($_POST['filter_cruise']) && $_POST['filter_cruise'] = 'filter_cruise'){
		$place = Input::post('place',0); $assign_list['place'] = $place;
		$star_number = Input::post('star_number',0); $assign_list['star_number'] = $star_number;
		$price_range_ID = Input::post('price_range_ID',0); $assign_list['price_range_ID'] = $price_range_ID;
		if($place > 0){
...
//	$clsCruise->setDeBug(1);
	$listCruise=$clsCruise->getAll($cond.$order_by,$clsCruise->pkey.',title,slug,star_number,about,image');
//	die;
...
function default_ajChooseCabinCruise(){
	...
	$promotion=$discount['discount_value']; $assign_list['discount_value']=str_replace('.','',$promotion);
	$discount_type=$discount['discount_type']; $assign_list['discount_type']=$discount_type;
	...
	if($type=='BOOKNOW'){
		...
		$dataCruise = Input::post('dataCruise','');
		$promotion = Input::post('discount_value',0);
		$discount_type = Input::post('discount_type',0);
		$number_cabin = Input::post('number_cabin',0);
		$str_departure_date=($departure_date != '')?strtotime($departure_date):''; 
		$arr_cabin = [];
		$SessionChooseCabin = [];
		foreach($dataCruise as $key => $value){
			if($value['cruise_cabin_id'] > 0){
				...
				$SessionChooseCabin[$cruise_itinerary_id]['discount_type'] = $discount_type;
				$SessionChooseCabin[$cruise_itinerary_id]['cruise_itinerary_id'] = $cruise_itinerary_id;
				$SessionChooseCabin[$cruise_itinerary_id]['number_cabin'] = $number_cabin;
				$SessionChooseCabin[$cruise_itinerary_id]['departure_date'] = $str_departure_date;
				if(!empty($SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id])){					
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['number_adult'] 	+= (int)$value['number_adult'];
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['number_child'] 	+= (int)$value['number_child'];
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['number_cabin'] 	+= (int)$value['number_cabin'];
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['totalprice'] 		+= (int)$value['totalprice']*(int)$value['number_cabin'];
				}else{
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['number_adult'] 	= (int)$value['number_adult'];
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['number_child'] 	= (int)$value['number_child'];
					$SessionChooseCabin[$cruise_itinerary_id]['cabin'][$cruise_cabin_id]['number_cabin'] 	= (int)$value['number_cabin'];
        ...
        if($clsISO->getCheckActiveModulePackage($package_id,'booking','booking_cruise','default')){
            ...
            vnSessionSetVar('ContactCruise',$SessionChooseCabin[$cruise_itinerary_id]);
