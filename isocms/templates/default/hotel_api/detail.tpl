<link rel="stylesheet" href="{$URL_JS}/venobox/venobox.css?v={$upd_version}" type="text/css">
<script src="{$URL_JS}/venobox/venobox.js?v={$upd_version}"></script>
{assign var=title_hotel value=$response_hotel_detail.data.name}
{assign var=intro_hotel value=$response_hotel_detail.data.descriptions}
    ...
    <nav class="breadcrumb-main  breadcrumb-cruise bg-default breadcrumb-more bg_fff">
        <div class="container">
            <ol class="breadcrumb hidden-xs mt0 bg_fff" itemscope itemtype="http://schema.org/BreadcrumbList">
               ...
               <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
				   <a itemtype="http://schema.org/Thing" itemscope itemprop="item" href="{$curl}" title="{$core->get_Lang('Hotels')}">
                ...
                <div class="row">
                    <div class="col-md-9">
                        <article class="hotels">
                            <img u="image" class="cover width100 heightAuto" src="{$clsHotel->getImage($hotel_id,848,360)}" alt="{$title_hotel}" />
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="body">
										<h1 class="pane-title pane-title-fixed text-left mb10 mt0">{$title_hotel} <img src="{$clsHotel->getImageStar($response_hotel_detail.data.star)}" alt="{$title_hotel}"/></h1>
                                        <div class="address"><i class="fa fa-map-marker"></i> {$response_hotel_detail.data.address}</div>
                                ...
                                <div class="col-md-4">
                                    <div class="needhelp">
                                        ...
                                        <p class="phone"><i class="fa fa-phone"></i>{$clsConfiguration->getValue('CompanyHotline')}</p>
                        ...
                        <article class="wrap">
                            <ul id="tabsk" class="clienttabs mtm">
                                ...
                                {if $lstHotelRoom[0].hotel_room_id ne ''}
                                <li><a href="javascript:void(0);">{$core->get_Lang('Hotel Rooms')}</a></li>
                                {/if}
                                {if $listImage[0].hotel_image_id}
                            ...
                            <ul class="tabs_content pl0" id="lstTabs" style="border:0 !important">
                                ...
                                <li class="contentTab TabHotel" style="display:none">
                                    ...
                                    <table class="tbl-grid width100 table0" style="min-width:600px; overflow:auto">
                                        <tr>
                                            ...
                                            {section name=j loop=$lstCol} 
                                            <td class="tb-header text-center" style="width: 60px">
                                                <strong>{$clsHotelPriceCol->getTitle($lstCol[j].hotel_price_col_id)} ({$clsISO->getRate()})</strong>
                                            </td>
                                            {/section}
                                        </tr>
                                ...
                                {/if}
                                {if $listImage[0].hotel_image_id ne ''}
                            ...
                            <div class="books_or_made mgt15">
                                <a class="booknow fl" href="{$clsHotel->getLinkBook($hotel_id)}">
                        ...
                        {if $lstHotelRelated[0].hotel_id ne ''}
                        <section id="relatedBox" class="hotelBox mt40">
                            ...
                            <div class="jcarousel-box owl-carousel" id="jcarousel-related-slides">
                                ...
                                {assign var=link value = $clsHotel->getLink($lstHotelRelated[i].hotel_id)}
                                {assign var=title value = $clsHotel->getTitle($lstHotelRelated[i].hotel_id)}
                                <article class="hotelItem">
                                    <div class="post-thumb">
                                        <a href="{$link}" class="photo" title="{$title}">
                                            <img class="img-responsive width100 heightAuto" src="{$clsHotel->getImage($lstHotelRelated[i].hotel_id,552,368)}" alt="{$title}"/>
                                        </a>
                                        <span style="cursor:pointer" class="{if $profile_id eq ''}exitLogin{else}addWishlist{/if} heart-o inline-block text-center btnwl" title="{$core->get_Lang('Saved to wishlist')}" clsTable="Hotel" data="{$lstHotelRelated[i].hotel_id}" id="addwishlistHotel{$lstHotelRelated[i].hotel_id}">{$clsHotel->getOneField('wishlist_num',$lstHotelRelated[i].hotel_id)}</span>
                                        <div class="figure">
                                            <a href="{$link}" title="{$title}" class="viewdetail">{$core->get_Lang('View Detail')}</a>
                                            <p class="price">{$core->get_Lang('From')} <span class="price-Inc">{$clsHotel->getPrice($lstHotelRelated[i].hotel_id)}</span></p>
                                        </div>
                                    </div>
                                    <div class="body">
                                        <h3 class="title"><a href="{$link}" title="{$title}">{$title}</a> <img class="star" style="height:13px; width:auto" src="{$clsHotel->getImageStar($lstHotelRelated[i].star_id)}" alt="{$title}" /></h3>
                                        <p class="address"><i class="fa fa-map-marker"></i> {$clsHotel->getAddress($lstHotelRelated[i].hotel_id)}</p>
                                        <p class="text" style="display:none">{$clsHotel->getIntro($lstHotelRelated[i].hotel_id)|strip_tags|truncate:120}</p>
                                    </div>
                                </article>
                                {/section}
                            </div>
