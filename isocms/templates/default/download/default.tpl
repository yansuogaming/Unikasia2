				...
				<div class="col-md-8 col-md-offset-2">
					<div class="Aboutcontent">
						...
						<ul class="list-brochure" id="listHolderView">  
							...
							{assign var=titleDownload value=$clsDownload->getTitle($listDownload[i].download_id,$listDownload[i])}  
							{assign var=introDownload value=$clsDownload->getIntro($listDownload[i].download_id,$listDownload[i])}  
							<li class="box bg_fff mb20" {if $smarty.section.i.iteration gt '4'}style="display:none"{/if}>
								<h3><a href="{$listDownload[i].attachment_file}" class="download" title="{$titleDownload}">{$titleDownload}</a></h3>
								{if $introDownload ne ''}
								<div class="formatTextIntro">{$introDownload}</div>
								{/if}
								<a href="{$listDownload[i].attachment_file}" class="download" title="{$core->get_Lang('Download')}">{$core->get_Lang('Download')} <span class="filesize">({$clsDownload->getFileSize($listDownload[i].download_id,"KB",$listDownload[i])}, {$clsDownload->getFileExtension($listDownload[i].download_id,$listDownload[i])})</span></a>
							</li>
							{/section}
