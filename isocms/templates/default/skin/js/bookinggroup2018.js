...
function tinhtoan(){
	...
	var price_deposit_vn_post = document.getElementById("price_deposit_vn_post");
	...
	if (!isNaN(tong)){
		...
		price_adult_html.innerHTML = price_adult;
		price_adult_post.value = price_adult;
		price_child_html.innerHTML = price_child;
		price_child_post.value = price_child;
		price_infant_html.innerHTML = price_infant;
		price_infant_post.value = price_infant;
		price_total_traveller.innerHTML = national_traveler;
		price_total_traveller_post.value = national_traveler;
		
		if(national_adults>1){
			for_number_traveller.innerHTML = For +' '+ national_adults +' '+ adults;
		}else{
			for_number_traveller.innerHTML = For +' '+ national_adults +' '+ adult;
		}
		if(national_child>0){
		...
		if(promotion_check >0){
			...
			if(LANG_ID=='vn'){
				price_total_amount.innerHTML = format_price_vn(national_surcharge - price_promotion_value) ;
				price_total_amount_post.value =  format_price_value_vn(national_surcharge - price_promotion_value);
				price_promotion.innerHTML =  format_price_vn(price_promotion_value);
				price_promotion_post.value =  format_price_value_vn(price_promotion_value);
			}else{
			...
			if(deposit.value >0){
				...
				if(LANG_ID=='vn'){
					price_deposit.innerHTML = format_price_vn(price_deposit_value);
					price_paynow.innerHTML = format_price_vn(price_deposit_value);
					price_paynow_post.value = format_price_value_vn(price_deposit_value);
					price_deposit_post.value = format_price_value_vn(price_deposit_value);
					
					price_deposit_vn.innerHTML = format_price_vn(price_deposit_post.value*exchange_rate,0);
					price_deposit_vn_post.value = format_price_vn(price_deposit_post.value*exchange_rate,0);
					surcharge_value_html.innerHTML = '$'+ format_price_vn(surcharge*(national-price_promotion_value)/100);
					surcharge_value_post.value =  format_price_value_vn(surcharge*(national-price_promotion_value)/100);
					price_remaining.innerHTML = format_price_vn(national_surcharge-price_promotion_value-(price_deposit_value));
					price_remaining_post.value = format_price_value_vn(national_surcharge-price_promotion_value-(price_deposit_value));
				}else{
				...
				if(LANG_ID=='vn'){
					surcharge_value_html.innerHTML = '$'+ format_price_vn(surcharge*(national-price_promotion_value)/100);
					surcharge_value_post.value =  format_price_value_vn(surcharge*(national-price_promotion_value)/100);
					price_remaining.innerHTML = format_price_vn(national_surcharge-price_promotion_value);
					price_remaining_post.value = format_price_value_vn(national_surcharge-price_promotion_value);
					price_deposit_vn.innerHTML = format_price_vn(price_remaining_post.value*exchange_rate,0);
					price_deposit_vn_post.value = format_price_vn(price_remaining_post.value*exchange_rate,0);
					price_paynow.innerHTML = format_price_vn(national_surcharge-price_promotion_value,0);
					price_paynow_post.value = format_price_value_vn(national_surcharge-price_promotion_value,0);
				}else{
					surcharge_value_html.innerHTML = '$'+ format_price(surcharge*(national-price_promotion_value)/100);
					surcharge_value_post.value =  format_price2(surcharge*(national-price_promotion_value)/100);
					...
					price_deposit_vn.innerHTML = format_price_vn(price_remaining_post.value*exchange_rate,0);
					price_deposit_vn_post.value = format_price_vn(price_remaining_post.value*exchange_rate,0);
					price_paynow.innerHTML = format_price(national_surcharge-price_promotion_value,0);
					price_paynow_post.value = format_price2(national_surcharge-price_promotion_value,0);
				}
			}
		}else{
			...
			if(deposit.value >0){
				if(LANG_ID=='vn'){
					price_total_amount.innerHTML =  format_price_vn(national_surcharge);
					price_total_amount_post.value =  format_price_value_vn(national_surcharge);
					...
					price_paynow_post.value = format_price_value_vn(price_deposit_value);
					price_deposit_post.value = format_price_value_vn(price_deposit_value);
					price_deposit_vn.innerHTML = format_price_vn(price_deposit_value*exchange_rate,0);
					price_deposit_vn_post.value = format_price_vn(price_deposit_value*exchange_rate,0);
					surcharge_value_html.innerHTML = '$'+ format_price_vn(surcharge*national/100);
					surcharge_value_post.value =  format_price_value_vn(surcharge*national/100);
					price_remaining.innerHTML = format_price_vn(national_surcharge-(price_deposit_value));
					price_remaining_post.value = format_price_value_vn(national_surcharge-(price_deposit_value));
				}else{
				...
				if(LANG_ID=='vn'){
					price_total_amount.innerHTML =  format_price_vn(national_surcharge);
					price_total_amount_post.value =  format_price_value_vn(national_surcharge);
					price_remaining.innerHTML =  format_price_vn(national_surcharge);
					price_remaining_post.value =  format_price_value_vn(national_surcharge);
					surcharge_value_html.innerHTML = '$'+ format_price_vn(surcharge*national/100);
					surcharge_value_post.value =  format_price_value_vn(surcharge*national/100);
	
					price_deposit_vn.innerHTML = format_price_vn(national_surcharge*exchange_rate,0);
					price_deposit_vn_post.value = format_price_value_vn(national_surcharge*exchange_rate,0);
					
					price_paynow_post.value = format_price_value_vn(national_surcharge,0);
					price_paynow.innerHTML = format_price_vn(national_surcharge,0);
				}else{
					price_total_amount.innerHTML =  format_price(national_surcharge);
					price_total_amount_post.value =  format_price2(national_surcharge);
					...
					price_deposit_vn.innerHTML = format_price_vn(national_surcharge*exchange_rate,0);
					price_deposit_vn_post.value = format_price_vn(national_surcharge*exchange_rate,0);
					
					price_paynow_post.value = format_price2(national_surcharge,0);
					price_paynow.innerHTML = format_price(national_surcharge,0);
				}
			}
		}
...
function createCustomerInfo(obj){
	...
	this.stt_input.appendTo($('<span class="mb20"><b>'+ Traveller +'</b> </span>').appendTo(this.div));
	this.title_input = $('<select class = "form-control form-booking_input title none_appearance">' +
			...
			'</select></span>').appendTo($('<span class="mb20"><label>'+ Title_optional +'</label>').appendTo(this.div));
	this.name_input = $('<input type = "text" class = "form-control form-booking_input"/>');
	this.name_input.appendTo($('<span class="mb20"><label>'+ FullName +'</label></span>').appendTo(this.div));
	
	this.birthday_input = $('<input type="hidden" class="example4">').appendTo($('<span class="mb20"><label>'+ DateofBirth +'</label></span>').appendTo(this.div));
	
	this.address_input = $('<input type = "text" class = "form-control form-booking_input" />').appendTo($('<span class="mb20"><label>'+ Address +'</label></span>').appendTo(this.div));
	
	this.gender_input = $('<input type="radio" class="radio" value="'+Male+'" checked/>'+Male+
	'<input type="radio" class="radio mgl50" value="'+Female+'"/>'+Female+
			'</span>').appendTo($('<span class="mb20"><label>'+ Gender +'</label>').appendTo(this.div));
			
	this.tourist_age_type_input = $('<select class = "form-control form-booking_input appearance_none" >' +
		'</select>').appendTo($('<span class="mb20"><label>'+ Traveller_Type +'</label></span>').appendTo(this.div));
...
function format_price_value_vn(n) {
	return n.toFixed(0).replace(/./g, function(c, i, a) {
		return i > 0 && c !== "." && (a.length - i) % 3 === 0 ? "." + c : c;
	});
}
