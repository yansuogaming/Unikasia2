...
return setTr;}
function setThemeCheckDate(pr_title,pr_class,pr_data_utc,pr_input,pr_fm_input){return'<div class="t-dates t-date-'+pr_class+'"><label class="t-date-info-title">'+pr_title+'</label><span class="text_day" id="text_day_'+pr_class+'"></span>'+
...
if(this_picker.find('.t-input-check-in').val()!=='null'||settings.toDayHighlighted!==false){toDayElement[i].className='t-start';loadTextDayCheckIn($('.t-input-check-in').val());}}
if(Number(dayselect)===dataUTC[1]){var this_picker=$(toDayElement[i]).parents('.t-datepicker')
if(this_picker.find('.t-input-check-out').val()!=='null'||settings.nextDayHighlighted!==false){toDayElement[i].className='t-end';loadTextDayCheckOut($('.t-input-check-out').val());$('#pick_travellers').trigger("click")}}
...
this_el_arrow.find('.t-arrow-top').css({'display':'none'})
