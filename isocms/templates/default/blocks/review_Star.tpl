...
<div class="stories bg_f7f7f7" id="stories">
	...
	<div class="read_stories">
		{if $mod eq 'tour' || $mod eq 'tour_new' || $mod eq 'voucher'}
						...
						<li id="Reviews{$lstReview[i].reviews_id}" class="box item" {if $smarty.section.i.iteration gt '3'} style="display:none"{/if}>
							<div class="member">
								<div class="image"><label class="rate-number text-normal">
									{$clsReviews->getRates($lstReview[i].reviews_id,$lstReview[i])}</label>
								</div>
								<div class="name">{$clsReviews->getFullname($lstReview[i].reviews_id,$lstReview[i])}</div>
							</div>
							<div class="body">
								<p class="inline-block full-width">
								<span class="rate inline-block fl full-width_450"><label class="rate-2019 text_left">{$clsReviews->getRatesStar($lstReview[i].reviews_id,$lstReview[i])}</label> &nbsp;<span class="btn_rate color_999">{$clsReviews->getNewRates($lstReview[i].reviews_id,$lstReview[i])}</span></span>
