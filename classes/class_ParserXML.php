 ...
 * xml_parser.php
 *
 * @(#) $Header: /opt2/ena/metal/xmlparser/xml_parser.php,v 1.37 2013/07/14 13:38:12 mlemos Exp $
 *
 */
...
<class>
	...
	<version>@(#) $Id: xml_parser.php,v 1.37 2013/07/14 13:38:12 mlemos Exp $</version>
...
{metadocument}
	<function>
		...
		<argument>
			...
			<out />
			<documentation>
				<purpose>Optional array with options to configure the behavior
					this function.<paragraphbreak />

					This array should contain entries with the names of the options
						and the respective values. Currently it supports the following
						values:<paragraphbreak />

					<tt>ignoreunknown</tt> - Boolean option to indicate that unknown
						elements not specified in the <argumentlink>
							<argument>types</argument>
							<function>ExtractElementData</function>
						</argumentlink> argument may be ignored, so their values will
						not be returned.<paragraphbreak />
					</purpose>
			</documentation>
		</argument>
		<do>
{/metadocument}
*/
	Function ExtractElementData($path, $name, $types, $hash, &$values, $options = array())
	{
		$values = array();
		$required = $types;
		if(!IsSet($this->structure[$path]))
			...
			if(GetType($data) == 'array')
			{
				...
				if(!IsSet($types[$type]))
				{
					...
					&& $options['ignoreunknown'])
						continue;
					return($this->SetPathErrorPosition($element_path, XML_PARSER_VALIDATE_DATA_ERROR, strlen($path) ? 'unexpected '.$name.' element "'.$type.'"' : 'the XML document is not a valid '.$name.' definition'));
				}
				$t = $types[$type];
				if(!IsSet($t['type']))
				...
				switch($expected_type)
				{
					...
					case 'hash':
						...
						if(strlen($error = $this->ExtractElementData($element_path, $type, $t['types'], $child_hash, $v, IsSet($t['options']) ? $t['options'] : array())))
							return $error;
		...
		if(count($required))
		{
			...
			for(Reset($required), $r = 0; $r < $tr; Next($required), ++$r)
			{
				...
				$minimum = (IsSet($t['minimum']) ? $t['minimum'] : 1);
				if($minimum > 0)
					return($this->SetPathErrorPosition($path, XML_PARSER_VALIDATE_DATA_ERROR, 'it was not specified the '.$name.' element "'.$element.'"'.($minimum > 1 ? ' at least '.$minimum.' times' : '')));
			}
		}
		return '';
	}
